...
void main() {
  ...
  group('v1 → 当前版本 增量迁移', () {
    ...
    test('迁移后 settings 结构可由当前版本生成类读取（v11 未改该表）', () async {
      final SchemaVerifier verifier = SchemaVerifier(GeneratedHelper());
      final InitializedSchema schema = await verifier.schemaAt(1);
      ...
      // 降级）而报「没有迁移策略」；而 settings 表在 v2→…→v11 里都没有变化，
      // 用当前版本的类验证同样覆盖它的列定义。
      final v11.DatabaseAtV11 check = v11.DatabaseAtV11(schema.newConnection());
      await check
          .into(check.settings)
          .insert(
            v11.SettingsCompanion.insert(
              key: 'SET-082',
              value: '{"level":"error"}',
            ),
          );
      final v11.SettingsData row = await check
