...
void main() {
  ...
  group('v1 → 当前版本 增量迁移', () {
    ...
    test('迁移后 settings 结构可由当前版本生成类读取（v5 未改该表）', () async {
      final SchemaVerifier verifier = SchemaVerifier(GeneratedHelper());
      final InitializedSchema schema = await verifier.schemaAt(1);
      ...
      // 用 v5 而不是更低的版本：迁移终点是 v5，用 v2/v3/v4 生成类打开 v5 库会因为
      // 版本不符失败；而 settings 表在 v2→v3→v4→v5 里都没有变化，用 v5 类验证同样
      // 覆盖它的列定义。
      final v5.DatabaseAtV5 check = v5.DatabaseAtV5(schema.newConnection());
      await check
          .into(check.settings)
          .insert(
      ...
      final v5.SettingsData row = await check
