...
void main() {
  ...
  group('v1 → 当前版本 增量迁移', () {
    ...
    test('迁移后 settings 结构可由当前版本生成类读取（v4 未改该表）', () async {
      final SchemaVerifier verifier = SchemaVerifier(GeneratedHelper());
      final InitializedSchema schema = await verifier.schemaAt(1);
      ...
      // 用 v4 而不是更低的版本：迁移终点是 v4，用 v2/v3 生成类打开 v4 库会因为
      // 版本不符失败；而 settings 表在 v2→v3→v4 里都没有变化，用 v4 类验证同样
      // 覆盖它的列定义。
      final v4.DatabaseAtV4 check = v4.DatabaseAtV4(schema.newConnection());
      await check
          .into(check.settings)
          .insert(
      ...
      final v4.SettingsData row = await check
