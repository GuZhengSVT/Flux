...
void main() {
  ...
  group('v1 → 当前版本 增量迁移', () {
    ...
    test('迁移后 settings 结构可由当前版本生成类读取（v8 未改该表）', () async {
      final SchemaVerifier verifier = SchemaVerifier(GeneratedHelper());
      final InitializedSchema schema = await verifier.schemaAt(1);
      ...
      // 用 v8 而不是更低的版本：迁移终点是 v8，用更低的生成类打开 v8 库会因为
      // 版本不符（被当成降级）失败；而 settings 表在 v2→…→v8 里都没有变化，用
      // 当前版本的类验证同样覆盖它的列定义。
      final v8.DatabaseAtV8 check = v8.DatabaseAtV8(schema.newConnection());
      await check
          .into(check.settings)
          .insert(
            v8.SettingsCompanion.insert(
              key: 'SET-082',
              value: '{"level":"error"}',
            ),
          );
      final v8.SettingsData row = await check
