...
// T031 改为 v11 基线，T034 改为 v12 基线）：迁移安全。
//
...
// 11（搜索服务记录表，T031）、12（文章表补 AI 摘要三列，T034）之后，本文件里的断言
// 相应改为 12，而「代码比库新但迁移写坏」的场景用**比当前版本再高一级**的坏实现模拟
// （现为 v13）；
...
void main() {
  ...
  group('迁移失败不重建数据库', () {
    test('升级步骤抛错：打开失败，原数据与版本号保持可回退', () async {
      ...
      expect(_userVersion(raw), 12, reason: '迁移失败不得推进版本号');
      expect(
