...
void main() {
  group('ARB 资源', () {
    ...
    test('条目数量与生成类一致（生成物来自当前 ARB）', () {
      ...
      // 886 + 18 = 904。
      // T047 新增 60 条存储清理文案（设置入口两条、页面标题、占用分类小节与五个分类名、
      // 分类行/合计/测量时间/媒体上限四条、刷新占用、读取失败、清缓存小节与边界说明、
      // 付费结果提示、预览/影响/空态/确认标题与正文与按钮与完成六条、自动清理小节与说明、
      // 三个开关名、保留天数、两个保护开关与警告、预览与立即执行与影响与全关说明与确认
      // 三条与完成、孤儿快照小节与说明与不可用与两个按钮与结果与完成六条、通用失败一条、
      // 彻底删除小节与说明与标题与影响与无关联与按钮与完成七条、阅读页入口一条）：
      // 904 + 58 = 962。
      // T048 新增 27 条诊断与恢复文案（设置入口两条、页面标题、导出小节与说明与确认标题与
      // 正文与按钮与导出按钮与完成与失败与保存路径九条、恢复编排小节与四种状态说明与恢复
      // 目录与旧目录与清理按钮与确认标题与正文与完成与放弃按钮与放弃回执与失败与回退十二条、
      // 通用失败一条）：962 + 27 = 989。
      expect(_messageKeys(zh).length, 989);
      expect(_messageKeys(en).length, 989);
    });
