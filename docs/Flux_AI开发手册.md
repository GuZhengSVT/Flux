...
| T019 | T004、T012、T013、T017 | 列表视图与正文阅读组件产品化 | 所有卡片模式、分页/虚拟化、目录/上下篇/页内查找、代码/公式/摘要完整性、原文不丢；滚动阅读及返回锚点验收 | DONE（受控文档渲染器移植并产品化：22 条解析用例 + 20 条渲染/目录/查找/集成用例 + 4 张 golden；LaTeX 用 flutter_math_fork 0.7.4 纯 Flutter 自绘，不支持命令显示原式与提示；**列表面板/瀑布流卡片形态与真正虚拟化未做**，滚动锚点仍属后续，详见 R019）|
...
### 7.1.12 轮次记录 R019（T019）
    ...
    费用与秘密：未发起任何真实 AI/搜索调用，无费用产生；无真实网络请求（渲染层完全不联网，
      图片也不加载）
    遗留问题与未运行项：
      1) **列表面板与虚拟化卡片形态未做**：架构第 7 节要求的卡片形态（紧凑/正常/宽松三档、缺图
         不占位、桌面瀑布流宽 280–360）、三栏布局的来源栏与列表栏、以及列表**虚拟化**仍沿用 T017
         的朴素单栏列表。本轮把正文侧做成了产品级，列表侧只补了「已脱离订阅」标注与快照传递；
      2) **返回锚点未实现**：详情页从顶部开始阅读，返回列表时筛选与页码保留但**滚动位置回到顶部**
         （T017 遗留第 5 条的延续）。需要 ScrollController 位置持久化，属列表产品化的一部分；
