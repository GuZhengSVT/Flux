...
- Key：曾在此处记录，因文档入库导致泄露，已于 2026-09-22 从仓库移除。该 Key 应视为已泄露并尽快在 DeepSeek 平台吊销轮换；新 Key 由维护者本机保存，不再写入任何仓库文件。历史提交中仍存在该 Key，处理方式见 R-LEAK。

### 9.2 R-LEAK 密钥泄露记录（2026-09-22）

事实：测试用 DeepSeek API Key 在 T006（066c08d）复制规划文档入库时被带入 docs/Flux_AI开发手册.md，并随该提交及后续推送公开于 GitHub。

影响范围：仓库全部历史（含 066c08d 之前由 d15a628 起算的推送历史中该文件的所有版本）；任何访问过仓库的人或爬虫均可能已收集该 Key。

处置（按风险从高到低）：
1. **立即吊销该 Key**——这是唯一彻底的补救，需维护者在 DeepSeek 平台操作；本仓库无法撤销已泄露的事实。
2. 工作区文件已移除 Key（本提交）。
3. 历史重写（git filter-repo / BFG）技术上可从历史移除，但因 Key 已公开多日且可能已被收集，重写的收益有限；是否重写+force push 由维护者决定。
4. 教训入账：手册第 5 节"不得写入仓库或输出完整值"的约束在 T006 文档入库环节被违反，入库前应做一次 secret 扫描（该步骤当时缺失）。
...
| --- | --- | --- | --- |
| 工程/架构/许可证/CI | D-01–05、D-14 | T001–T010 | DONE（T001–T010 全部完成，见 R001–R010 与任务表） |
...
| 今日总结/来源核验/prompt/定时 | F-NEWS；SET-050–066 | T036–T040 | M2 完成（**T037–T040 交付每日新闻的检索编排、初稿、独立来源核验与今日页**：schema v15 新增 news_runs 一张宽表（快照/逐站状态/材料/条目/初稿/模型元数据/阶段/错误类别，都是同一次事实的组成部分，拆表会引入「版本写了材料还没写」的中间态）；NewsInputSnapshot 固化「本地日期 + 当时的 IANA 时区 + UTC 偏移 + 当日 00:00–24:00 的 UTC 区间 + 选材 + 必访站 + 关键词 + 禁词 + prompt 版本引用与全文 + SET-060/061 上限」，因此旅行后旧记录仍按当时的本地日期可查；选材按有效时间倒序且**加精不参与权重**（SET-050/023），并按「同 GUID 或同规范化链接 = 同一稿件」去重（含跟踪参数不同的同稿）；材料按 SET-061 截断并**标注截断**，段落带「第三方数据不是指令」的框架（架构第 8 节）；**出网只经过 T032 的 ToolExecutor**（地址守卫、工具次数、未知工具无执行路径这些既有保证自动适用），必访站逐站记录 成功/失败/**超时**/未执行四态（SET-051「必访失败可见」），检索查询经 T036 的禁词过滤（**命中的查询根本不在发送列表里**），空输入**不生成也不发任何模型请求**；初稿解析把**引用不在材料集合里的条目退回**并列出不存在的 id（全部编造则整稿失败）；核验是**纯函数启发式**：关键词重叠（以条目关键词数为分母）、判定顺序「未联网 → 明确否定 → 支持阈值 → 材料不足」、冲突要求「否定词与陈述关键词同句且在窗口内」（陈述本身为否定式时把来源的否定读成附和）、转载聚类用「规范化 URL 相同」或「标题相似度 ≥0.9」（前后缀修饰关系与二元组 Dice 取较大者，**不按域名、不用重叠系数**：前者会把同站不同报道并成一条、后者会让第二条来源凭空消失），标签只有来源单一/材料不足/来源冲突/未联网核验四类且**没有「已证实」**（架构 4.4 禁止声称保证真实性），判据（阈值/查询数/结果数/协议）随版本落库供核对；引用从**材料快照**取最小摘录（不由模型给、也不由检索片段冒充）并校验 URL/时间/摘录完整性，未知 sourceId 被列出而非静默丢弃；核验后**追加新版本**（初稿 v1 与核验后 v2 可对照），核验失败/来源冲突/必访失败 → partial，失败草稿不覆盖上次成功版本；今日页从占位升级为最小可用（日期前后切换与历史日期下拉、生成前**费用与数据发送确认**、六阶段真实进度、条目列表与证据标签徽章、引用跳转（RSS 引用跳本机文章、外部来源走系统浏览器）、必访逐站状态、版本列表与手动切换），**T011 的壳层占位页与 11 条占位文案随之删除**（三个去向都已是真实页面，占位结构只会误导；断点规则仍在 shell_layout.dart 并由用例逐点钉住 599/600/1099/1100）。**T039 把今日页从最小可用升为产品化**：近 7 天条带（有记录的日期带圆点、今天用「今天」标出）+ 日历弹层回溯历史日期（上限锁到今天），翻到历史日期时明确说明「历史记录归属生成当时的时区、不因现在换时区而改写」；同日多版本连同**时间与模型**列出并可切换，可删除**非当前**版本（确认对话框；当前版本被 store 与界面双重拒绝）；生成进度在六阶段之外**逐站**显示必访站的成功/超时/失败/跳过/**待获取**（新增 onSiteResult 逐站回调，界面拿到真实执行结果而不是按站点数推算），取消后按钮进入「正在取消…」并禁用（`cancelling`）；引用跳转区分本机文章与外部来源，**正文已被清理**的本机引用显示「原文已清理，只保留最小摘录」（判据是「本机仍在、完整性声明本应有正文、而正文为空」，不是「正文为空」——源只给摘要的文章不该被说成被清理过），并在版本顶部汇总本日有多少条引用已被清理；**九态逐态映射**（无结果/材料不足/生成中/失败/取消/中断/等待配置/等待网络/排队），有失败记录但无成功版本时显示「本次生成状态 + 中止阶段」而不是假装「这一天没有生成过」。**T040 交付默认开启的定时总结**：SET-056/057 默认开/20:00；时间规则是纯函数并按「关闭 → 等待配置 → 今日已成功 → 未到点 → 到点」判定（等待配置先于到点，因此界面从早到晚一致；跨日不补是「今天还没到点且没成功」的自然结果）；waitingConfiguration **真的零请求**且四种原因各有不同文案；费用告知必须持久化且是本机项（没有它就等于未经告知地后台付费运行，同步到别处等于让另一台设备自动付费）；任务开始写 running 占位、启动时把遗留的 running 标 interrupted 且**不重放**、成功清占位/失败取消原地收尾；补跑是新任务与新版本；单并发闸门；设置页显示开关/时点/下次运行/今日已完成/等待配置原因与带正文的费用告知确认。**M2 出口**：T025–T040 全部完成即 M2 闭环，出口口径为「目标协议/供应商按验证矩阵交付、真实搜索/必访/证据链、视觉降级、十分钟与五次规则、定时默认开启且可控」；其中**除 DeepSeek 外无真实凭据实测**（7.3 逐行 NOT_RUN），不把接口完成说成实测通过；**macOS 无后台执行保证**，应用退出后不运行，实际保证只有「应用在运行时到点会跑」与「下次启动补跑当天错过的一次」。**仍未实现**：同步（T041 起）与结果缓存（T047 范围））|
| WebDAV 共通设置与阅读状态 | SET-070–075 | T041–T045 | DOING（**T041 交付同步的数据出境边界与跨设备身份**：schema v16 新增四张表（单行 sync_state 共同基线/本地修订/能力探测三态、**字段级**待同步变更供 T043 判断可合并与必须选版、墓碑、订阅别名）并给 articles 加 syncKey 与条件唯一索引；纳入/排除清单**从 SET 注册表的 C/D/S 分类派生**（手抄清单会与文档第 6 节漂移，表现为「某项 C 类设置在新设备上永远是默认值」而界面上看不出来），S 类秘密与 D 类设备项逐条排除、**首发还排除正文/摘要/图片/统计**；跨设备文章键 = SHA-256(源 syncId + 类型标记 + 证据)，**类型标记进摘要**因此「GUID 与链接同串」「可靠与不可靠指纹」都得到不同的键，三个证据都缺失时返回 null 而不是编一个键；订阅对齐先 syncId 再按**唯一的**规范化 URL（有歧义时不对齐而不是挑一个，挑错会把远端状态写进另一条订阅）；缺正文时落 remote 占位行（状态落库、正文为空、靠 syncKey 被后续抓取匹配），本机已有该文章时只应用状态、**正文与身份依据一律不动**；抓取补身份只允许 remote → 真实依据。**T042 交付 WebDAV 能力探测与不可变快照传输**：能力探测是四步事实的纯函数判定，最后一步「过期 ETag 必须被拒」是唯一能识破**忽略 If-Match 的服务器**的实验（那类服务器能通过前三步，只在多端并发时静默互相覆盖），判成 readOnlyPull 后 publish 一个字节都不写；快照按**内容哈希**命名（名字即内容），manifest 把指针与元数据字段分开（**不比较设备墙钟**，架构 5.2），读不懂的 manifest 返回 null 而不是当成「没有当前版本」；发布流程 = 读基线 → 上传快照（判重）→ **读回校验哈希** → 条件写 manifest（首次 If-Match: *）→ **412 重读重试 ≤ 3 轮** → 本地确认，全程**不碰数据库**；快照已写而 manifest 未更新 → 孤儿快照（不是当前版本，有判据）；客户端只做协议动作（密码只进 Basic 头、响应体限长、XML **先拒 DTD/ENTITY**、412 翻译成 conflict 而不是异常）。测试用**状态化**内存 WebDAV 服务器（45 条 + T041 的 64 条）；**未对真实 WebDAV 服务器做任何调用（NOT_RUN，无凭据）**；**T043 交付三方合并/条件发布/墓碑/冲突**：合并是**纯函数**（签名里没有任何时间参数，因此「仅比较设备墙钟」在结构上不可能被顺手加进来）——双方同值保留、仅一方变更取该方（判据是与共同基线比较）、**同字段并发变更才是冲突**（两个候选原样返回，不做 OR、不自动取胜方；用户选版后形成新版本）；**字段缺失 ≠ 值为 null**（一侧没报告某字段按「与基线相同」处理，否则远端只报名字会清掉本机的排列权重）；**本机墓碑优先于远端复活**、**远端破坏性删除只登记不应用**（未批准不清本地数据）。**SyncEngine** 按架构原话编排：读基线 → 上传快照 → 读回校验哈希 → 条件写 manifest → **412 重读重合并重试 ≤3 轮**（第二轮基于**新读到的**父版本）→ 本地事务确认；**确认按修订号上界**，写回前逐字段查「是否在本轮修订之后又被改过」，命中则保留本机新值（**上传期间的新修改不丢失**）；**崩溃恢复幂等**的判据是「远端当前版本的内容哈希 == 本机当前内容」（版本号由内容派生，因此是精确判据），命中只补一次本地确认、**不重复上传**；降级服务器**一个字节都不写**。**T044 交付同步设置页、触发排队与首次合并**：四个触发来源（启动 SET-072 / 30min 间隔 SET-073 / 变更防抖 5s / 手动）走**唯一入口**，运行中再次触发只记一个待跑标记（**同机不并发**）并在结束后串行续跑；**未配置或关闭时一个请求都不发**；**能力只探测一次**；**首次同步先预览**（SET-075：没有共同基线且远端已有内容时不自动发布，给出项数与差异，默认 **preferLocal 不覆盖本地**，确认后才建立基线）；**冲突由用户逐条选版**（manual 下引擎不发布；选完形成新版本，成功发布后状态里的冲突被清掉）；设置页有 URL/用户名/**密码（Keychain、不回显、写完清空输入框）**/远端目录/设备名、**只读的测试连接**（只 PROPFIND + GET，界面上写明不会写入）、同步范围（**清单来自 T041 的投影**，不手抄）、手动同步、状态（最近同步/待同步数/冲突数）、降级提示条与首次合并/冲突卡片。**真实 WebDAV 双客户端验证仍为 NOT_RUN（无凭据）**；Mock 双客户端测试（内存服务器 + 两个隔离内存库）是 fixture 证据；**T045 交付删除与收藏保留的跨设备集成**：远端破坏性删除的**登记与应用彻底分开**（T043 登记，本轮用 core/domain/remote_deletion.dart 的纯规则应用），影响范围只复用 T018 的预览数字、确认框默认沿用远端当时的选择、本机没对齐或类别不认识的删除**拒绝执行**（不猜目标）；**未确认不清数据**是结构性的（preview 只读，用例断言预览后一行不变）；删除操作元数据（保留收藏的选择）随墓碑进入快照，因此别的设备能先展示影响范围；**已删除条目不复活**在合并层（墓碑优先）之外补上**刷新层**的墓碑过滤（跳过的源进报告 skipped，不是静默丢弃；读取失败或未接线时放行）；本机显式重新添加同一源会忘掉旧墓碑（否则那个源永远跳过刷新且无任何线索）；**占位行升级**接线到 ArticleStore：按同步键匹配到 remote 占位行后补正文/身份与同步键，**阅读状态与收藏保留**；界面以独立状态显示「正文尚未同步」（卡片徽标 + 详情页横幅） |
| 明文备份/恢复/清理/诊断 | SET-076–082 | T046–T048 | DONE（**T046 交付明文备份导出与安全恢复到新目录**：一致性快照用 SQLite 的 VACUUM INTO（不是复制活动库文件，因此 WAL 里已提交的内容不会丢）、条目白名单只有「快照/清单/media」三类（凭据在规则层进不了包）、恢复先校验后写新目录且失败不动原库、ZIP 与 Python zipfile 双向互操作。**T047 交付分类占用、一键清缓存、自动清理与缓存限额**：占用在用例层合并「数据库聚合 SQL + 媒体文件系统」两个来源并把元数据与半成品字节也算进媒体类；清缓存只删可再生内容（媒体缓存、AI 结果缓存、失败任务草稿），**不动**付费成功结果、阅读状态、收藏、订阅、prompt 与秘密（逐项断言 + 逐列断言无凭据列）；自动清理按 SET-077 三个独立开关与天数（默认全关时不访问存储）、SET-078 的收藏/later 保护默认生效且被跳过篇数可见；**正文释放只写正文列**，身份与状态留在原处，因此刷新仍匹配同一行而不复活成未读（用真实 upsertArticles 再抓同一 GUID 断言）；**schema v17** 给 `ai_result_cache_records` 加 `byte_length`（迁移按 UTF-8 字节回填，与 Dart 口径一致）与 `last_used_at`（不回填），条数 200 + 字节 128 MiB 双上限、写入后按 LRU 淘汰（判定在 core 的纯函数里）；孤儿快照 GC 在 T042 判据上再加时间与引用集合两个条件（刚上传或拿不到服务器时间的一律不回收）。**T048 交付恢复编排与脱敏诊断导出**：状态机 `pendingRestart → switched → cleaned`，标记写在数据目录父目录、启动时在打开数据库之前执行；切换是两步目录改名（停放 → 启用），第 2 步失败**自动回退**并保留标记，回退失败明确报错；「已切换」阶段绝不再切，恢复目录不可用时丢弃标记并保持当前数据目录不变；旧目录删除需用户确认。诊断包四节各有**字段白名单**（白名单外字段直接丢掉），因此「包里有原文/prompt/凭据」在结构上不可表达——没有任何一节读过正文、输入快照或 prompt，同步摘要不报地址与用户名、系统信息只报数据目录存在性；值逐条脱敏后整篇再跑一遍（覆盖只在拼装后才出现的 `名字=值` 形状）。**M3 出口达成。**） |
| 平台体验/资源占用/可访问性 | 架构第 7、8 节 | T049–T052 | DONE（**macOS 范围**；Android 专项未做）。T049 键盘/右键/语义/回归已交付（REVIEW 待走查项见 R049）；T051 图标资源范围 DONE（全设置审计部分未做）；T052 macOS 验收矩阵/profile 复测/菜单栏已交付（Android 逐项 NOT_RUN），见 R052。性能基线（原 R050 执行的 macOS 部分）已归入 T052 |
| GitHub 更新与正式发布 | SET-083、084 | T053–T056 | TODO（开发暂停；**预发布构建已按用户指令完成**：macOS release .app 31MB 与 Android arm64 release APK 27.0MB 均已产出并交付检查，见 R-BUILD；正式签名/公证/发布仍需维护者密钥与授权） |
...
| M2 AI、搜索与新闻闭环 | T025–T040 | 所有目标协议/供应商按验证矩阵交付；真实搜索/必访/证据链、视觉降级、十分钟/五次规则、定时默认开启且可控。 | **DONE（闭环）**：T025–T040 全部完成（R025–R040）。出口口径与保留项：协议/供应商按 7.3 的验证矩阵逐行记录（**除 DeepSeek 外无凭据，逐行 NOT_RUN**，不把接口完成说成实测支持）；搜索/必访/证据链、视觉文本降级、10 分钟总时限与五次规则、定时默认开启且可控（默认开/等待配置零请求/补跑当天一次/终止记中断）均已有本机用例与构建证据。**未做**：真实凭据下的端到端实测（需用户提供凭据）、Android 工程与两平台正式签名（属后续任务）。 |
| M3 同步、恢复和清理 | T041–T048 | **DONE（出口达成，T041–T048 全部完成）**：双设备共通设置与三态/收藏同步、冲突不丢数据、删除与收藏保留的跨设备集成、明文备份可恢复且凭据不泄露、分类占用/一键清缓存/自动清理/缓存与快照限额、恢复编排与脱敏诊断导出。出口口径为「目标协议与同步规则按验证矩阵交付、清缓存不删状态与付费结果、诊断包无原文/prompt/凭据」；**真实 WebDAV 双客户端验证为 NOT_RUN（无凭据）**，Mock 双客户端测试只作为 fixture 证据，不宣称真实服务器兼容。 |
...
| T039 | T019、T020、T038 | 今日页日期/历史/生成进度/来源跳转 | RSS 引用跳本地文章、外部源外开；已清理内容有说明；无结果/资料不足/生成中/失败/取消各状态与模型元数据齐全 | DONE（缺失 REVIEW）：今日页从「最小可用」升为产品化（见 R039）。**日期与历史**：顶部「近 7 天」条带（有记录的日期带圆点，今天用「今天」而不是数字标出，可点击切换）+ 日历弹层可回溯任意历史日期（上限锁到今天：新闻总结只能为当天生成，选到未来只会看到空白）；翻到历史日期时用一条说明讲清「历史记录归属生成当时的设备时区，不会因现在换时区而改写」（架构 4.4）——这条说明是必要的，因为查不到记录时的默认解释是「我的总结丢了」。**版本管理**：同日各版本连同**时间与模型**列出（时间按记录快照里冻结的 UTC 偏移换算，不用设备当前时区，否则同一条记录在两台设备上会显示两个时间）；可对照切换，可删除**非当前**版本（带确认对话框，说明「当前展示的版本不会被删除」），删除当前版本被 store 层与界面双重拒绝。**生成进度**：六阶段真实回调 + 必访站**逐站**显示成功/超时/失败/跳过/待获取（新增 `onSiteResult` 逐站回调，界面拿到的是真实执行结果而不是「按站点数推算的进度」；计划站先以「待获取」列出，慢站卡住时用户能看出「还有两个没轮到」）。**取消**：取消后按钮进入「正在取消…」并禁用（新增 `cancelling` 状态）——取消不是瞬时的，提前切回「生成」会让用户在任务仍在跑时看到「没在生成」。**来源跳转**：RSS 引用跳本机文章、外部引用走系统浏览器（沿用 T020 的外开安全逻辑）；**正文已被清理**的本机文章显示「原文已清理，只保留最小摘录」（判据是「本机仍存在、完整性声明本应有正文、而正文为空」，**不是**「正文为空」——源只给摘要的文章不该被说成「被清理过」），跳转前先提示，历史版本顶部另给出「本日 N 条引用的正文已被清理」的汇总。**状态完备**：九态逐态映射（无结果/材料不足/生成中/失败/取消/中断/等待配置/等待网络/排队），有失败记录但没有成功版本时显示「本次生成状态 + 中止阶段」而不是假装「这一天没有生成过」；中断记录明说「不会自动重发」。**无 schema 变更**（新增的删除与中断标记都在 v15 表内）；新增 31 条文案 |
| T040 | T016、T029、T036–T039 | 默认开启的定时总结与后台调度 | 默认 20:00、10 分钟；waitingConfiguration 无网络、当天补跑一次、已有成功不重复、跨时区/双设备成本告知、受系统终止记中断 | DONE（缺失 REVIEW）：定时总结**默认开启**（SET-056 默认 true、SET-057 默认 20:00，见 R040）。**时间规则是纯函数**（core/domain/news_schedule.dart）：`evaluateDailyNewsDue(now, policy, ready, completedLocalDate, lastAttemptUtc)` 按固定顺序判定并返回一个**结论 + 下一次时间**——顺序本身是设计：**关闭优先**（用户明确说这台设备不要自动跑，就不去提配置缺失）、**等待配置先于到点判定**（未配置时从早到晚都显示「等待配置」，而不是 20:00 之前显示「按计划运行」、到点那一刻才变成缺配置）、**今日已成功优先于到点**（当天成功版本存在则不重复自动收费，判据是「今天」而不是「最近一次运行」，因此昨天成功过不影响今天）、**到点即跑**（`now >= 计划时刻`，这一条同时实现「运行中每分钟检查到点」与「启动时补跑当天错过的时点」）、**跨日不补**（昨天错过就放弃：它不是一条额外判断，而是「今天还没到点且今天没成功过 → notDue」自然成立，昨天那次机会随日期键翻页消失）。计划时刻由「当地读数的年月日 + SET-057 的时分」交给注入的 SessionLocalZone 换算，因此夏令时切换日的偏移由时区实现决定（D-07）。**waitingConfiguration 真的零请求**：齐备性判定在读时间之后、跑任务之前，缺模型/缺 Key/模型列表读失败/未确认费用告知各有一个**不同的原因标识**，界面逐因给出不同的下一动作（去「AI 服务」加模型、去补 Key、去确认告知）——合成一句「等待配置」会让用户不知道该配什么。**费用告知必须持久化且是本机项**（device.newsCostNoticeAcknowledged，与引导标记、视觉发送告知同一做法）：定时任务没有任何对话框，没有这条记录就等于未经告知地后台付费运行；同步到另一台设备会让那台设备因一条从别处来的记录而自动开跑并自己付费（架构 4.4「执行开关是本机项」）。数据库不可用时降级实现**读作未确认**（fail-closed）。**占位行让中断可见**：任务开始先写一条 running 占位（**不动** is_current），启动时把上次留下的 running 一次 UPDATE 标成 interrupted（**不自动重放**，沿用 T030 的口径；失败/取消则把占位行原地收尾，成功路径清掉占位行，不留永远 running 的孤儿行）；补跑是**新任务与新版本**，不是把旧行改写成成功。**并发闸门只有一个**：启动检查与每分钟检查都经过 runIfDue，运行期间直接返回（两处各判一次会让同一分钟内两次触发同时进入并争同一个版本号）。**设置页接线**：定时小节显示开关（SET-056，写入后立即重评估）、时点（SET-057，时间选择器 + 保存回执）、下次运行时间（按评估时固化的偏移换算）、今日已完成/正在运行/已关闭，以及带正文的**费用告知确认**；缺搜索只提示不阻塞（RSS 与必访站仍能产出，核验标注「未联网核验」）。**macOS 无后台执行保证**：应用退出后不运行，「20:00 一定会跑」不成立；能保证的是「应用在运行时到点会跑」与「下次启动补跑当天错过的一次」，这条写在界面说明与文档里而不是含糊过去。新增 25 条文案。 |

### M3：同步、恢复、清理

| ID | 前置 | 任务与范围 | 交付与验收条件 | 状态 |
| --- | --- | --- | --- | --- |
| T041 | T009、T010、T014、T017 | 同步数据 schema/稳定文章键/C-D-S 投影 | 仅共通设置+订阅/状态/收藏，设备项/凭据完全排除；独立导入对齐 Feed、缺正文保存状态占位、GUID 异源不冲突 | DONE（缺失 REVIEW）：见 R041。**投影是纳入/排除的权威清单且从注册表派生**（C 类且可持久化才纳入，因此清单不会与文档第 6 节漂移），S 类秘密与 D 类设备项逐条排除，**排除项里还包含订阅的本机 id、credentialRef 与四个抓取诊断列，以及文章的正文/正文哈希/提取正文/源摘要/AI 摘要/图片/阅读秒数**（首发不同步内容，完整备份用于人工搬迁）。**跨设备文章键 = SHA-256(源 syncId + 类型标记 + 证据) 前 32 位**：类型标记进摘要，因此「GUID 与链接恰好同串」「可靠与不可靠指纹」都得到不同的键；三个证据都缺失时返回 **null** 而不是编一个键（否则两台设备上的无关文章会因同一个空输入互相覆盖状态）。**订阅对齐先 syncId、再按唯一的规范化 URL**：URL 有歧义（本机重复添加过）时**不对齐**而不是挑一个，一条本机订阅只被对齐一次，且只在 URL 匹配时保存别名（下次同步不必再赌 URL）。**缺正文占位**：远端状态到达而本机没有该文章时，落一行 identityBasis=remote 的占位行（状态落库、**正文为空**、syncKey 由远端键派生以便后续抓取匹配），applyRemoteArticleState 里**没有任何写正文的语句**；本机已有该文章时**只应用状态**（正文/标题/身份依据一律不动，避免用远端的空正文清空已抓取的全文）；抓取补上身份时只允许 remote → 真实依据（**绝不反向**，否则下一轮刷新会把它当新文章再插一行）。**私密 URL 拆分复用 T015 的 stripUrlSecrets**（秘密参数清单只有一处），其他设备「需要补填凭据」的判据是「确实剥离了秘密」而不是「地址看起来需要认证」。schema v16 新增 sync_state_records（**单行**共同基线/本地修订/能力探测三态）、sync_pending_changes（**字段级**粒度，T043 的合并依据）、sync_tombstones（跨设备不得复活）、sync_feed_alias_records，并给 articles 加 syncKey + 条件唯一索引；**确认按修订号上界**（上传期间的新改动必须留下），**墓碑不随同步成功清除**，迁移**不回填**任何待同步变更或墓碑。新增 64 条断言 |
| T042 | T041 | WebDAV 能力探测与不可变快照传输 | schema/大小/hash 校验、唯一快照、强 ETag/If-Match 检测、断网/部分上传/旧 schema；服务器不满足条件写则禁多写自动覆盖 | DONE（缺失 REVIEW）：见 R042。**能力探测是四步事实的纯函数判定**，顺序本身是设计：写不成功 → 服务器不给 ETag → 带 If-Match 的写被拒 → 写入后 **ETag 未变**（固定/弱 ETag 会让「条件写」退化成「无条件覆盖」）→ **过期 ETag 未被拒绝**。最后一步是关键：一台**忽略 If-Match** 的服务器能通过前三步检测，只有在多端并发时才会静默互相覆盖，因此必须用「过期 ETag 写一次」这个实验把它识破；判成 readOnlyPull 后 `publish` **一个字节都不写**（有断言核对该路径上 PUT 计数不变），能力未探测（unknown）同样 fail-closed。**不可变快照按内容哈希命名**（名字即内容），因此 manifest 指向的版本与快照内容的关系可校验；`SyncManifest` 把**指针字段与元数据字段分开**，revision/deviceName/publishedAt 不参与「谁更新」的判定（架构 5.2 禁止仅比较设备墙钟），`decode` 对缺字段/类型错/**更高协议版本**一律返回 null（不用默认值凑一个父版本，也不按旧形状解读新格式）；远端 manifest 读不懂时 `readManifest` **明确失败**而不是当成「没有当前版本」（后者会让本机覆盖掉一份看不懂的版本）。发布流程按架构原话的顺序落地且**不碰数据库**（网络不在长事务内）：读 manifest+ETag → 上传快照（PROPFIND 判重，内容寻址因此同名必然同内容、重传无意义）→ **读回校验哈希** → 条件写 manifest（首次用 `If-Match: *` 表达「远端还没有当前版本」）→ **412 重读重试 ≤ 3 轮**（第二轮基于**新读到的**父版本，有断言）→ 读回校验 manifest → 返回结论让上层在本地事务里确认；读回哈希不一致时**绝不把 manifest 指过去**；三次 412 用尽则放弃且远端 manifest 从未被写入。**孤儿快照不是当前版本**：快照已写而 manifest 未更新时返回 orphanSnapshot（本机状态不变），并提供 `isCurrentSnapshot`/`orphanSnapshots` 判据（被引用的上一版仍要作为合并基线保留）。客户端只做协议动作：PROPFIND（Depth 0/1）/GET/PUT（If-Match）/DELETE/MKCOL（405 视为已存在），**密码只进 Basic 认证头**（不进 URL/日志/错误消息，有断言），响应体两道限长，失败路径也读干响应流以复用连接，地址走 configuredSource 策略（用户显式配置的端点允许内网），**412 翻译成 conflict 而不是抛错**。XML 解析**先拒 DTD/ENTITY**（与 T013/T015 同一条防线，独立实现）再限深度/节点数，`href` 归一化把完整 URL/percent 编码/末尾斜杠归到同一路径，**没有 href 的 response 被跳过**（不猜一个路径）。测试用**状态化**内存 WebDAV 服务器（文件表 + ETag + If-Match + 412 + 断网 + 可继承以模拟四类服务器行为），**45 条**覆盖探测六种判定、XML 防护、请求往返、发布协议、冲突重试、孤儿快照与 manifest 编解码；**未对真实 WebDAV 服务器做任何调用（NOT_RUN，无凭据）** |
| T043 | T042 | 三方合并/条件发布/墓碑/冲突 | read/later/unread 冲突不可 OR；取消收藏/时钟偏移、412/并发重试≤3、删除不复活、新 schema 阻写；同步期间新本地修改不丢失，远端发布后本机崩溃可幂等恢复 | DONE（缺失 REVIEW）：见 R043。**三方合并是纯函数**（签名里**没有任何时间参数**，因此「仅比较设备墙钟」在结构上不可能被后来者顺手加进来）：双方同值保留、仅一方变更取该方（判据是与**共同基线**比较）、**同字段并发变更才是冲突**（两个候选都返回，不自动取胜方）。三态不可 OR 由「候选值就是原值、没有任何合成逻辑」保证；布尔只有两个取值，因此「双方都偏离基线且互不相同」在布尔上不可能，**取消收藏的真冲突来自删除操作元数据 keepFavorites**（有用例钉住）。**字段缺失 ≠ 值为 null**：一侧没报告某字段按「与基线相同」处理（否则远端只报名字就会把本机的排列权重清掉），显式 null 才算一次真实变更。**删除走墓碑且墓碑优先于远端复活**；**远端破坏性删除只登记不应用**（进 pendingRemoteDeletions，合并结果里也不含那条实体，因此本机不会用一次上传推翻远端的删除）。**SyncEngine** 按架构原话编排：读基线 → 上传快照 → 读回校验哈希 → 条件写 manifest → **412 重读重合并重试 ≤3 轮** → 本地事务确认；412 之后基于**新读到的**父版本重合并（第二轮发布的是基于新基线的结果，有用例断言父版本链连续）；3 轮用尽 → waitingRetry 且**远端 manifest 从未被本机改写**。**确认按修订号上界**：写回每个字段前查「该字段是否在本轮修订之后又被改过」，是则跳过并记入 preservedLocalFields——上传期间的新修改必须留下（有用例在第一次写入的回调里改名字并断言新名字没被旧值覆盖）。**崩溃恢复幂等**：判据是「远端当前版本的内容哈希 == 本机当前内容」（版本号由内容派生，因此这是精确判据），命中则只补一次本地确认、**不重复上传**（断言快照集合与远端版本都没变）。**降级服务器只读**：能力不是 conditionalWrite 时**一个字节都不写**（断言 PUT 计数不变），只把远端快照读下来供显式导入。**时钟偏移无影响**：两台设备相差一年、互相超前落后的用例里，结果只由基线决定。新增 64 条断言 |
| T044 | T043、T040 | 同步设置页、触发排队/首次合并 | 启动/30min/变更防抖/手动串行；首次预览不覆盖，冲突可选版、取消/恢复；两个隔离客户端真实 WebDAV 验证 | DONE（缺失 REVIEW）：见 R044。**SyncManager 把四个触发来源收成一个入口**（启动 SET-072 syncOnStart / 30min 间隔 SET-073 / 变更防抖 SET-072 的 5 秒 / 手动）：运行中再次触发**只记一个待跑标记**而不开第二轮（同机不并发），结束后**串行续跑**那次被合并的触发（有用例用可阻塞的传输装饰器确定地构造在途窗口，断言并发触发返回 null、快照只产生一份）。**防抖每次重新计时**，因此连续改动只同步一次；**manualOnly 不起定时器**；**未配置或关闭时一个请求都不发**（断言 requestLog 为空）。**能力只探测一次**（结论记在状态里复用）。**首次合并预览**（SET-075）：没有共同基线且远端已有内容时**先预览不写**（断言 PUT 计数不变），给出远端/本机项数、差异处数、待确认删除数，默认策略是 **preferLocal（不覆盖本地）**；用户确认后建立基线并发布，本机内容仍在（并集）。**冲突选版**：manual 下引擎**不发布**，管理器把冲突列表交给界面逐条选版，`resolveConflicts` 之后形成新版本；**成功发布后状态里的冲突被清掉**（引擎仍在结果里保留「这次发生过哪条冲突」这个事实，但界面读的是「还有多少事要用户做」）。**设置页**（我的 → 同步与备份）：URL/用户名/密码（Keychain，**不回显已存密码**、写完立刻清空输入框）/远端目录 flux-v1/设备名（SET-070）、**测试连接是只读探测**（只 PROPFIND + GET，界面上写明「不会写入」；有断言请求方法集合恰为 {PROPFIND, GET}）、同步范围（**清单来自 T041 的投影**，只展示数量与说明，不手抄）/两个范围开关（SET-074）、手动同步按钮、当前状态（最近同步/待同步数/冲突数/能力未探测）、降级模式提示条、首次合并卡片与冲突卡片（字段/本机值/远端值/选择控件）。**降级提示条由状态驱动**。新增 38 条断言（设置读取与 URL 校验 9、管理器触发排队/首次合并/冲突 12、设置页组件 6、能力探测与只读探测 10、设置入口导航 1）；真实 WebDAV 双客户端验证按 6.3 记为 NOT_RUN（无凭据），Mock 双客户端测试作为 fixture 证据|
| T045 | T018、T043、T044 | 删除与收藏保留跨设备集成 | 删除订阅的选择进入操作元数据；远端破坏性变更本机预览确认，未确认不清数据；已删除条目不复活，正文未同步不虚构 | DONE（缺失 REVIEW）：见 R045。**远端破坏性删除的「登记」与「应用」彻底分开**：T043 只登记（pendingRemoteDeletions），本轮新增 core/domain/remote_deletion.dart 的**纯规则**做应用——RemoteDeletionTarget 由快照实体类别判定（订阅/分组/**不认识**），未知类别一律 unsupported 并**拒绝执行**（猜一个目标去删是不可撤销的错误，表现是「另一条订阅消失了」）；RemoteDeletionImpact 的影响范围只接受 T018 的 FeedDeletionPreview/GroupDeletionPreview（**数字只有一套算法**，分组按组内订阅逐条累加，因此与「逐个删源」一致）；确认框的默认勾选**沿用远端当时的选择**（否则「远端说有 3 篇收藏留下」与「本机删完只剩 0 篇」会对不上）；applyConfirmed 拒绝不可应用的目标（类型化失败而不是假成功——假成功会让界面把这条划掉而数据一行没动）。**未确认不清数据是结构性的**：preview/previewAll 只读，用例断言预览后文章数与订阅数都不变。**删除操作元数据进同步**：新增 infrastructure/local/deletion_sync_recorder.dart 的 recordLocalDeletionFact（**必须在事务内**调用）写墓碑 + 待同步标记 + 推进本地修订号（修订号由本机分配），T018 的 deleteFeed 与删除分组的**两个分支**都在同一事务内调用它；读快照时把 T018 deletion_events 的 keepFavorites 按 syncId 关联到墓碑上，因此**保留收藏的选择随快照离机**。**已删除条目不复活**覆盖两条路径：合并层沿用 T043 的墓碑优先，**刷新层**新增墓碑过滤（FeedTombstoneReader 窄端口 + refresh_scheduler 在派发前拦下，跳过的源进 RefreshRunReport.skipped 且 reason 为 tombstoned，不是静默丢弃；读取失败或未接线时按「没有墓碑」**放行**——刷新是只读的，不能让无关接线疏忽停摆全部刷新）。**本机显式重新添加同一源**时先忘掉旧墓碑（syncId 是规范化地址的摘要，重新添加必然撞上同一个键；不忘记会让那个源永远跳过刷新且没有任何线索）——这不是「复活」，那一条约束的是别的设备的旧快照。**占位行升级**：在三条身份规则之后加**按同步键匹配**这第四条路径（候选两个键：本机这次算出的键、以及由它派生的占位键），命中且 identityBasis 为 remote 才接管，补上标题/正文/身份依据与**同步键**，readingState 与 favorite 一律不动；真实身份的行不会被这条路径改写。**界面**：同步设置页的远端删除卡片从「只读说明」改为**确认区**（影响范围先可见 + 保留收藏勾选 + 确认按钮；不可应用项不给按钮并说明原因；确认后回执可见），文章卡片与详情页以独立状态显示「正文尚未同步」（key 为 article-card-body-not-synced / detail-body-not-synced，与「正文为空」「仅摘要」分别可见）。新增 20 条用例（预览不改数据、确认后保留收藏分支与全清分支、未对齐拒绝执行且一行不动、未知类别不猜、分组两分支、墓碑+待同步+修订号、快照带 keepFavorites、应用远端删除也留墓碑并写下本机选择、重新添加忘墓碑、墓碑跳过刷新且进 skipped、读取失败/未接线放行、占位行升级保状态与同步键、无证据不合并、真实身份不被改写、缺单行状态抛错）+ 3 条组件层（确认卡片的只读性与确认后结果、卡片与详情页的占位行状态）；全库 2033 tests 绿。**真实 WebDAV 双客户端验证仍为 NOT_RUN（无凭据，延续 R044）** |
| T046 | T009、T010、T041 | 明文完整备份与安全恢复 | 排除秘密、含/不含媒体、风险告知、WAL 一致性、ZIP 路径/体积/schema/hash 校验；恢复到干净目录成功，失败保留原库 | DONE（缺失 REVIEW）：见 R046。**导出**用 SQLite 的 VACUUM INTO 取一份**一致性快照**（不是复制活动数据库文件——那样会漏掉 WAL 里已提交但未落盘的页，架构 5.3 点名过这个缺陷；选中它的理由与 sqlite3_backup 的对比写在 backup_content_source.dart 顶部）；快照 + 版本清单 + 每文件 SHA-256 + 可选媒体打进 ZIP，**条目白名单**只有「快照 / 清单 / media 前缀」三类（白名单而不是黑名单：黑名单要穷举所有秘密形态，漏一种就等于把它写进包），因此「凭据进包」在规则层就不可表达；快照读自本机数据库，而库**没有任何凭据列**（SET-071/SET-027 只住 Keychain）；含媒体开关（SET-076）排除 .part 半成品，且导出用的临时快照在 finally 里删掉（不留一份用户不知道的明文副本）。**风险告知前置**：导出前必须先过确认对话框（「明文备份包含个人订阅与正文，任何持有者均可读取」），确认后才弹保存面板。**恢复**：选文件 → 校验 →**二次确认** → 写**新目录** → 校验新库 → 切换。校验顺序是「名字安全 → 白名单 → 清单可读 → schema 不高于当前 → 快照存在 → 大小与 SHA-256 逐个相符 → 清单之外无多余条目」，全部**先于任何写操作**；路径穿越拒绝四类（绝对路径、..、反斜杠、盘符）并额外拒绝空段与点号。更新的 schema 一律拒绝（旧版本不能写较新 schema），更旧的备份允许恢复（由 SQLite 打开时按真实迁移策略升级）。**失败不动原库是结构性的**：用例与实现里没有任何写到当前数据目录的语句，恢复目标只接受新目录或空目录（非空直接拒绝）；**写入完成不等于恢复成功**，因此写完必须真的 openAppDatabase 打开一次并核对 user_version。ZIP 编解码**自实现**（core/archive/zip_archive.dart，只用 dart:io 的 ZLibCodec(raw)，不新增依赖），因为读取侧必须能在**落地之前**拒绝路径穿越——第三方解压器先落盘再交给调用方检查就晚了；已与 Python zipfile 双向互操作（含非 ASCII 名、DEFLATE、CRC）。**界面**：设置 → 同步与备份 页新增备份小节（风险告知常驻可见 + 含媒体开关 + 导出/恢复两个按钮 + 预检信息（备份时间/schema/文章数/订阅数/是否含媒体）+ 二次确认 + 进度 + 失败按稳定类别名展示，且每次都写明「当前数据未被改动」）。新增 30 条用例（导出内容与清单哈希、含/不含媒体、取消、临时文件清理、**WAL 一致性**（写入后立即导出、52 篇逐行核对）、路径穿越、白名单、损坏 ZIP、错包版本、schema 更新、哈希/大小不符、清单条目缺失、目标目录非空、恢复后数据完整（订阅/正文/收藏/later）、媒体按开关落地、无效库校验失败、清单编解码确定性、ZIP 与 Python 双向互操作与失败分支）+ 6 条组件层；全库 2069 tests 绿 |
| T047 | T018、T021、T030、T038 | 存储分类、清理策略与缓存限额 | SET-077–082、收藏/later 默认保护、主动删订阅例外、引用最小快照、用户彻底删除关联结果；清缓存不删状态或付费结果 | DONE（缺失 REVIEW）：见 R047。**分类占用**在用例层合并两个来源——数据库五类（正文/总结/其他缓存/数据库）由 `DriftStorageCleanupStore.measureDatabase` 用聚合 SQL 统计（**只数非空正文**：空串与 NULL 都不算，否则「3000 篇只占 12 MiB」无法解释），媒体那一类由 `ImageCacheService.diskUsage` 提供并把**元数据与半成品 `.part` 的字节也算进去**（只报图片字节会让用户看到的合计与目录大小对不上）；媒体端口失败**不导致整页失败**（其余四类照常可读，失败那一类按 0 显示并留诊断），因为「少一类」比「看不到任何分类」好。**一键清缓存**（SET-079）只删可再生内容：媒体缓存全清、AI 结果缓存全清、**失败任务草稿**（failed/cancelled/interrupted 三类终态显式列出，新增终态不会被误判成草稿）清理；**不动**付费成功结果（news_runs 与 succeeded 任务的产出）、阅读状态、收藏、订阅、prompt 与秘密，有用例逐项断言（含「缓存与任务表里没有任何凭据列」的逐列检查）。**自动清理**（SET-077 三个独立开关 + 天数，默认全关）按媒体（最后访问 mtime）、**文章正文**（`COALESCE(published_at, fetched_at)`）、总结（created_at）三类各自判定；三个开关全关时**不访问存储**直接返回空影响（默认状态不该为一件不会发生的事扫库与缓存）；保护规则（SET-078）收藏与 later 默认排除、两个开关**互相独立**，且被跳过的篇数必须出现在预览里（用户据此看出开关真的生效）。**正文释放保身份是 UPDATE 语句的形状保证的**：只写 `body`/`body_hash`/`extracted_*` 与 `body_completeness`（改成 summaryOnly——库里确实只剩标题与摘要），`reading_state`、`favorite`、`guid`、`normalized_link`、`fallback_fingerprint`、`sync_key`、`feed_id` 一律不出现在补丁里，因此刷新按同一身份规则匹配到**同一行**；用例用真实 `upsertArticles` 再抓一次同一 GUID，断言「没有插新行、状态仍是 read、正文照常写回」。**ai_result_cache 容量上限补齐**（T030 遗留）：**schema v17** 加 `byte_length`（**迁移回填**，用 `length(CAST(result_text AS BLOB))`——`length(text)` 是字符数，中日韩会低估约三分之一，与 Dart 侧 `utf8ByteLength` 口径一致并有迁移用例逐字符类比对）与 `last_used_at`（**不回填**：历史行没有「曾经被读过」这个事实，用 created_at 顶上等于伪造一次命中）；条数 200 + 字节 128 MiB 双上限，每次成功写入后按 **LRU** 淘汰（命中一次就刷新使用时刻——只按创建时间淘汰是 FIFO，会删掉「每天都在用、第一次生成在半年前」的条目），判定住在 core 的纯函数 `planAiCacheEviction` 里，因此「淘汰顺序确定」「本次刚写入的条目不被删」「从未被读过的按写入时刻参与淘汰」这些性质能被纯 Dart 逐条断言。**孤儿快照 GC**（T042 遗留）：在 T042 的「不是当前版本」判据上再加**时间**维度并排除被引用集合——刚上传、manifest 还没写的快照**不回收**（它与正在发布的另一台设备赛跑），拿不到 `getlastmodified` 的也**不回收**（不猜）；预览（dryRun）只报告不删除，缺服务器时间的数量必须出现在报告里（否则「列得出但删不掉」完全不可见）；未配置同步时返回 `Ok(null)` 而不是错误。**彻底删除**（架构 5.3「不能留下隐藏副本」）：先只读列出关联范围（引用/译文/会话/图片缓存/新闻引用）再执行；单事务内删会话 → 译文（段落级联）→ 引用**置空而不删除**（架构 4.4 要求引用保留最小摘录作为历史总结的出处）→ 删文章本体；图片缓存按**地址哈希**算出文件名后在事务外回收（文件删除没有事务语义，把「文件被占用」混进事务会回滚掉已成功的数据库删除）。**界面**：新增设置 → 存储与清理页（分类占用含**测量时间**——一个没有时间的数字无法让用户判断它是否还对得上；媒体上限有参照；清缓存预览 → 确认（明说只删可再生内容与付费结果需另行处理）→ 回执；三个开关 + 天数步进（只落在 1/7/30/90/365/3650 档位，越界在结构上不可达）+ 两个保护开关（打开时给出明确警告）与自动清理预览/立即执行；孤儿快照检查/回收；阅读页新增「彻底删除本文」入口，确认框先列关联范围再执行）。新增 43 条用例（test/core/storage_cleanup_test.dart 14 条纯规则、test/core/ai_cache_bounds_test.dart 10 条淘汰规则、test/features/settings/cleanup_test.dart 19 条真实库与真实临时目录）与 4 条组件层、1 条 v16→v17 迁移、1 条 v17 快照断言；全库 2119 tests 绿 |
| T048 | T029、T043、T046、T047 | 故障恢复/诊断包与隐私审计 | 磁盘不足/崩溃/迁移中断/Key 失效/后台终止恢复；诊断预览脱敏，备份/同步/日志/分享无秘密样本泄漏 | DONE（缺失 REVIEW）：见 R048。**恢复编排**把 T046 的「需重启」半自动部分接起来：状态机 `pendingRestart → switched（确认后）→ cleaned`，标记写在数据目录的**父目录**（写成数据目录内部会让「数据目录被改名」这一步顺带把标记搬走，而它恰恰要在搬移中一直可见），启动时在**打开数据库之前**执行（先开旧库再改名会让数据库文件在使用中被搬动）。切换 = 停放旧目录 → 启用新目录，两步都是目录改名（**不用删除**：改名失败时旧目录还在、可回退）；第 2 步失败**自动搬回来**并保留 `pendingRestart` 标记（下次启动可重试），回退本身失败则**明确报错**而不是静默停在停放位置。「已切换」阶段再启动**绝不再切一次**（那会把正在用的目录搬到别处），只返回 `awaitCleanupConfirmation`；恢复目录不存在或库不可用时丢弃标记并保持当前数据目录不变（`dropMarkerAndReport`）；`RestoreMarker.decode` 对任何结构问题返回 null（一个凑出来的标记会让应用把数据目录切到不存在的位置），四类判定是 core 的纯函数 `decideRestoreAction`。旧目录删除必须用户确认（`completeCleanup` 只在 switched 阶段生效，其它阶段返回 `Ok(null)`），删不掉时**保留标记**（丢掉会让旧目录变成没有任何线索的孤儿）。搬移的目标已存在时**拒绝**（`Directory.rename` 在目标为空时成功、非空时失败，两种行为都不可依赖；静默合并不可撤销）。**诊断导出**（SET-082）：四节内容各有**字段白名单**（系统信息 / 存储统计 / 同步状态摘要 / 日志），白名单之外的字段直接丢掉，因此「包里有原文或 prompt」**在结构上不可表达**——系统信息只报平台事实与「数据目录是否存在」（不报路径，路径里有用户名），同步摘要只报能力三态与计数（**不报服务器地址与用户名**，它们合起来足以说明是谁在用哪个服务），存储统计只报字节数与计数，**没有任何一节读过 `articles.body` / `ai_tasks.input_snapshot` / `news_runs.prompt`**；值逐条脱敏后**整篇再跑一遍**（拼装会引入「单看每段都干净、连起来形成 `名字=值`」的形状，有用例用只经拼装才出现的秘密形状钉住这一层）；导出前确认页明说包内有什么、不含什么，保存走 T015/T046 的既有文件端口（不新增一条能任意写盘的通道），文件名只带日期（设备名会出现在邮件标题里，是第二条说明「这是谁的机器」的线索）。**界面**：新增设置 → 诊断与恢复页（导出小节 + 恢复编排状态展示，三种阶段各自说明「当前数据在哪」，只在有停放目录时给出「删除旧数据目录」，pendingRestart 阶段只给「放弃这次恢复」——放弃只清标记、**不删**恢复出来的目录）。新增 40 条用例（test/core/restore_orchestration_test.dart 11 条纯规则、test/core/diagnostics_export_test.dart 10 条白名单与脱敏、test/features/settings/restore_orchestration_test.dart 11 条真实目录与真实库、test/features/settings/diagnostics_export_test.dart 8 条真实四节内容）与 4 条组件层、1 条设置入口贯通；全库 2164 tests 绿 |
...
| T050 | T040、T044、T048 | Android 专项验收 | Android 17 + 目标 ARM64，返回/旋转/进程杀死/锁屏/权限/Keystore/网络限制、10min 被系统打断；目标 API 配置实际核对 | **android/ 工程已初始化并产出 release APK**（R-BUILD：arm64 27.0MB，包 ID io.github.guzhengsvt.flux，label Flux）；本行验收项（返回/旋转/进程杀死/锁屏/权限/Keystore 等）仍 NOT_RUN——需真机测试，不因构建成功冒充通过。性能验收集部分（macOS）已归入 T052，见 R052 |
...
### 7.1.33 轮次记录 R039（T039）

    轮次/日期：R039 / 2026-09-22
    任务 ID 与状态变化：T039（今日页日期/历史/生成进度/来源跳转的产品化打磨）TODO → DONE（缺失 REVIEW）
    相关决策/功能/SET 项：架构 4.4（日期与展示采用设备时区、历史保存日期+时区、必访失败可见、
      引用只可使用真实 sourceId、保留上次成功版本）、架构 5.3（清缓存只释放正文、保留身份与状态）、
      架构第 7 节（每页状态要求）、D-07、SET-050、SET-051、SET-060、SET-066
    修改文件与主要行为：
      - core/domain/news_run.dart：NewsRunStore 增加 deleteVersion（**当前展示版本不可删**、
        不存在的版本报「缺失」而不是静默成功）；
      - infrastructure/local/news_run_store.dart：实现 deleteVersion（先读出目标再用一条说得清楚的
        错误拒绝当前版本，避免进入事务后回滚）；降级实现明确失败；
      - features/news/application/news_run_service.dart：新增 onSiteResult 逐站回调（界面拿到的是
        真实执行结果，而不是按站点数推算的进度）；
      - features/news/application/news_run_providers.dart 与 app/app_providers.dart：构造器
        typedef 与组合根接线补上 onSiteResult；
      - features/news/application/news_today_controller.dart：状态增加 today/recentDates/sites/
        plannedSites/clearedArticleIds/cancelling 与派生（latest 含失败记录、isToday、pendingSites、
        clearedCitationCount）；新增 deleteVersion、recentDateKeys（纯日期键往返，不读系统时区）、
        _probeClearedArticles（探测「本应有正文而正文为空」的本机引用；**探测失败按无法判断处理**，
        不显示说明也不报错——把它显示成「原文已清理」会让用户以为自己内容被删了）；cancel 现在把
        状态置 cancelling；
      - features/news/presentation/news_today_page.dart：头部改为两行（日期导航 + 主操作 / 日历入口 +
        近 7 天条带），窄窗下用 Expanded+Wrap 换行而不是溢出；新增历史日期说明横幅、逐站进度（含
        「待获取」）、`_noUsableResult` 九态状态区（含中止阶段）、版本条目含时间与模型、删除确认框与
        回执、引用缺摘录与正文已清理说明、已清理引用汇总横幅；时间戳按快照冻结的 UTC 偏移换算；
      - l10n：新增 31 条今日页产品化文案（中英各一份）；l10n_test 的条目数 743 → 774；
      - 测试：新增 test/features/news/news_today_controller_test.dart（6 条：近 7 天跨月、派生状态、
        pendingSites、clearedCitationCount）与 news_today_flow_test.dart（10 条：日期切换与「今天不随
        翻页移动」、历史日期按时区查旧记录、版本列表/删除/切换、六阶段顺序与逐站实时结果、取消落
        cancelled 与 cancelling 呈现、无输入落 failed、取消不覆盖上次成功版本）；扩展
        news_today_page_test（+10 条）与 news_run_store_test（+3 条版本删除）；news_run_test 的内存
        store 替身补齐 setCurrentVersion 的真实语义（替身无条件成功会让「切换失败」路径得到假绿）
        与 deleteVersion；
      - 更新三张壳 golden（默认去向就是今日页，头部布局变化是预期结果；已人工对照浅色宽窗/窄窗与
        深色宽窗三张截图确认，而非「跑一下让它变绿」）。
    数据迁移/删除/依赖变化：**无 schema 变更**（删除与中断标记都落在 v15 的 news_runs 表内，
      未新开 v16）；无删除功能或数据；无新增依赖；更新三张壳 golden。
    环境：macOS 27.0 (26A428) / Apple M4 / 16 GiB / arm64；Flutter 3.47.0 / Dart 3.13.0；debug（macOS）
    检查（均为本机实际执行，命令 | 退出码 | 结论 | 证据）
      flutter pub get | 0 | PASS | Got dependencies
      dart run build_runner build | 0 | PASS | Built with build_runner/aot in 7s；wrote 672 outputs
      dart format lib test | 0 | PASS | 无格式差异
      flutter analyze | 0 | PASS | No issues found
      flutter test | 0 | PASS | 1750 通过 / 2 跳过 / 0 失败（相对 T038 的 1720 新增 30 条）
      flutter build macos --debug | 0 | PASS | build/macos/Build/Products/Debug/Flux.app
    费用与秘密：**未发起任何真实网络调用，无费用产生**（生成与抓取全部走端口替身与 fixture，
      模型为脚本化替身）；新增文案与状态说明不含任何地址、Key 或文章正文
    遗留问题与未运行项：
      1) **缺失 REVIEW**：本轮改动集中在界面与状态呈现，判据（「已清理」的识别口径、逐站进度的
         呈现顺序）未经产品复核；
      2) **「正文已清理」的判据是启发式的**：只对 `sourceBody/extracted` 且正文为空的本机文章下结论；
         「文章已被彻底删除」与「读取失败」都不下结论（不显示说明），因此**可能漏报**清理——
         漏报比误报更符合这里的代价（误报会让用户以为内容被删）；
      3) **日历弹层上限锁到今天**：新闻总结只能为当天生成，因此不允许选未来日期；若将来支持补生成
         历史日期，需要同时定义「用哪一天的文章材料」；
      4) **逐站进度是运行中的即时状态**：生成结束后由版本记录接管，两者内容一致但来源不同（回调 vs
         落库事实）；界面不把运行中的结果写库；
      5) **未接真实 AI 与搜索调用**：除 DeepSeek 外无凭据（7.3 逐行 NOT_RUN），因此「长任务下进度与
         取消的真实观感」无实测；
      6) **未接入定时执行**（T040）与**未接结果缓存**（T047 范围）；7) Android 工程仍未初始化；
      8) 本轮成果未 push 到远端。
    需求是否变化、维护者是否批准：未改变任何验收条件文字；只更新任务状态列（T039 → DONE，标注
      缺失 REVIEW）、功能账本与 R039 记录。未删除任何功能或数据（新增的版本删除是**用户主动操作**，
      且当前版本受保护）。
    提交/差异范围：提交 "T039: today page productization with history, per-site progress and source jumps"；
      基线为 T038 的提交（aa740f6）。未 push
    下一可执行任务及前置条件：T040（默认开启的定时总结与后台调度），前置 T016/T036–T039 均已 DONE

### 7.1.34 轮次记录 R040（T040）

    轮次/日期：R040 / 2026-09-22
    任务 ID 与状态变化：T040（默认开启的定时总结与后台调度）TODO → DONE（缺失 REVIEW）
    相关决策/功能/SET 项：架构 4.4（定时默认 20:00、启用开关默认开、后台尽力执行、错过只补当日一次、
      当天成功版本存在则不重复自动收费、多设备各存自己的总结且可能产生多份费用、执行开关是本机项、
      总时限 10 分钟）、架构 4.5（受系统终止记 interrupted、不重放不确定是否计费的请求）、D-07、D-08、
      SET-056、SET-057、SET-058、SET-059、手册 6.3「定时」节
    修改文件与主要行为：
      - 新增 core/domain/news_schedule.dart（纯函数时间规则：DailyNewsPolicy/DailyNewsDueKind/
        DailyNewsDue/parseTimeOfDayMinutes/formatTimeOfDay/evaluateDailyNewsDue；判定顺序
        「关闭 → 等待配置 → 今日已成功 → 未到点 → 到点（catchUp）」）；
      - 新增 features/news/application/daily_news_scheduler.dart（调度器：启动检查、每分钟检查、
        单并发闸门、不取消在途任务、状态发布与 onStatus 回调；NewsCostNoticeStore 端口）；
      - 新增 features/news/application/daily_news_settings.dart（读 SET-056/057 + 设备时区；
        loadDailyNewsReadiness 逐项判定模型/凭据/搜索/费用告知，四种原因标识）；
      - 新增 features/news/application/daily_news_run.dart（DailyNewsRunner：开始写 running 占位
        （不动 is_current）→ 跑编排 → 成功清占位 / 失败与取消原地收尾为终态；占位写失败不阻止运行）；
      - 新增 features/news/application/daily_news_providers.dart（调度器、费用告知记录、状态 Provider）；
      - 新增 features/news/presentation/{daily_news_automation.dart,news_schedule_settings.dart}
        （宿主在 app_shell 里包在 Scaffold 外，与刷新宿主同批；设置页小节显示开关/时点/下次运行/
        今日已完成/等待配置原因/费用告知确认，缺搜索只提示不阻塞）；
      - 新增 infrastructure/local/news_cost_notice_store.dart（本机 device. 命名空间；降级实现读作
        未确认）；device_state_repository.dart 增加 newsCostNoticeAcknowledged 键；
      - news_run.dart 的 NewsRunStore 增加 reserveRunning / completeReserved / markRunningAsInterrupted，
        news_run_store.dart 实现三者（占位不动 is_current、收尾缺行则插入、中断只标 running 且不改 is_current）；
      - app_providers.dart 接线调度器（runInput 每次运行现造服务；markInterruptedOnStartup 接在同一张表上）；
      - l10n：新增 25 条定时文案（中英各一份）；l10n_test 条目数 774 → 799；
      - 测试：新增 daily_news_schedule_test（18 条纯函数：解析/格式化、默认开但初始不联网、
        按时区运行与换时区重算、错过补一次与跨日不补、今日已成功不重复、终止不伪称完成）、
        daily_news_scheduler_test（15 条调度器：到点触发、等待配置零请求、配置补齐自动解除、
        启动补跑、跨日不补、标中断只做一次且失败不阻塞补跑、单并发闸门、dispose、关闭不跑、
        自定义时间、状态发布）、daily_news_run_test（4 条占位行生命周期）、news_schedule_settings_test
        （7 条设置页接线）；扩展 news_run_store_test（+5 条占位与中断）；新增 daily_news_scheduler 的
        内存 store 替身补齐 reserveRunning/completeReserved/markRunningAsInterrupted。
    数据迁移/删除/依赖变化：**无 schema 变更**（占位行与中断标记都落在 v15 的 news_runs 表内，
      `running` 与 `interrupted` 是既有的九态取值，未新开 v16）；无删除功能或数据；无新增依赖。
    环境：macOS 27.0 (26A428) / Apple M4 / 16 GiB / arm64；Flutter 3.47.0 / Dart 3.13.0；debug（macOS）
    检查（均为本机实际执行，命令 | 退出码 | 结论 | 证据）
      flutter pub get | 0 | PASS | Got dependencies
      dart run build_runner build | 0 | PASS | Built with build_runner/aot in 7s；wrote 696 outputs
      dart format lib test | 0 | PASS | 无格式差异
      flutter analyze | 0 | PASS | No issues found
      flutter test | 0 | PASS | 1799 通过 / 2 跳过 / 0 失败（相对 T039 的 1750 新增 49 条）
      flutter build macos --debug | 0 | PASS | build/macos/Build/Products/Debug/Flux.app
    费用与秘密：**未发起任何真实网络调用，无费用产生**（时间规则是纯函数，调度器与占位行的用例全部用
      假时钟与内存 store）；诊断只记结构事实（原因类别、本地日期、触发来源），不记配置内容与凭据
    遗留问题与未运行项：
      1) **缺失 REVIEW**：本轮交付的是「默认开启 + 到点运行 + 等待配置不发请求」这套产品行为，
         其中的取舍（默认开、只有确认告知后才自动付费运行）未经产品复核；
      2) **macOS 上「后台执行」不成立**：应用退出后没有任何东西在跑，因此「每天 20:00 一定会生成」
         是**错的**。实际保证只有两条：应用在运行时到点会跑；错过的时点在下次启动补跑一次（仅当天）。
         这也是为什么跨日不补是正确取舍——补三天的费用用户没法预期。
      3) **10 分钟总时限未在本轮实测**：定时路径复用 T029 的 AiTaskBudget（SET-059 默认 10 分钟，
         含排队/抓取/模型尝试/核验/保存前校验），其规则由 T029 的假时钟用例覆盖；但「一次真实定时
         运行是否在 10 分钟内收敛」需要真实凭据才能测，属 7.3 的 NOT_RUN；
      4) **双设备成本告知只在文案层**：设置页与告知正文写明「这是本机项、每台设备各自调度可能产生
         多份费用」，但**没有**引导流程去提示用户关闭非主设备（架构 4.4 提到的那一步属后续引导任务）；
      5) **时区实现是固定偏移**（沿用 T023/T037 的 DeviceLocalZone，不引入时区库）：有夏令时的地区在
         切换当天，20:00 的实际时刻可能差一小时；中国大陆等无夏令时地区不受影响。这是既有边界，
         本轮不改，如实记录；
      6) **未接真实 AI 与搜索调用**：除 DeepSeek 外无凭据（7.3 逐行 NOT_RUN），因此「定时任务在真实
         凭据下是否真的能跑完并产出」无实测；7) **未接结果缓存**（T047 范围）；8) Android 工程仍未初始化；
      9) 本轮成果未 push 到远端。
    需求是否变化、维护者是否批准：未改变任何验收条件文字；只更新任务状态列（T040 → DONE，标注
      缺失 REVIEW）、功能账本与 R040 记录。**M2 出口说明**：T025–T040 全部完成（T039 见 R039、T040 见
      本记录），即 M2 闭环；出口口径与保留项为「架构 2.2 列出的 M2 目标协议/供应商按验证矩阵交付、
      真实搜索/必访/证据链、视觉降级、十分钟与五次规则、定时默认开启且可控」——其中**除 DeepSeek 外
      无真实凭据实测**，7.3 逐行标 NOT_RUN，不把接口完成说成实测通过。
    提交/差异范围：提交 "T040: default-on scheduled summary with waiting-configuration gate and catch-up"；
      基线为 T039 的提交（5b0c6e3）。未 push
    下一可执行任务及前置条件：M2 已完成（T025–T040），下一阶段任务见任务表（T041 起）

### 7.1.35 轮次记录 R041（T041）

    轮次/日期：R041 / 2026-09-22
    任务 ID 与状态变化：T041（同步数据 schema/稳定文章键/C-D-S 投影）TODO → DONE（缺失 REVIEW）
    相关决策/功能/SET 项：架构 5.1（Feed/ArticleState 与 Settings/SyncState 实体）、架构 5.2（WebDAV 首发
      范围、跨设备文章键、独立导入对齐 Feed、缺正文占位、私密 URL 拆分）、架构 5.3（统计首发不跨设备相加）、
      SET-027、SET-070–075、手册 6.3「身份」「同步」节
    修改文件与主要行为：
      - 新增 core/domain/sync_projection.dart（**纳入/排除的权威清单**）：
          * 纳入 = 注册表 C 类且可持久化的编号（**派生**而不是手抄，清单不会与文档第 6 节漂移）、订阅字段
            （syncId/normalizedUrl/name/sourceName/groupSyncId/favorite/enabled/newsEnabled/
            refreshIntervalMinutes/sortOrder）、分组字段、文章**状态**字段（syncKey/readingState/
            favorite/deleted/revision）、T036 的新闻规则（必访站 + 三个有序列表 + 中英 prompt 版本）；
          * 排除 = S 类秘密（SET-027/031/039/071）、D 类设备项（SET-003/005/006/008/056/072/073/074
            等）、订阅的本机 id 与 credentialRef 与四个抓取诊断列、文章的正文/正文哈希/提取正文/源摘要/
            AI 摘要/图片/阅读秒数；
          * planSyncFeedUrl：复用 T015 的 stripUrlSecrets 把私密地址拆成「可同步地址 + 本机凭据引用 +
            被剥离的参数名」，**其他设备补填**的判据是「确实剥离了秘密」而不是「地址看起来需要认证」；
      - 新增 core/domain/sync_article_key.dart（**稳定文章键与订阅对齐**）：
          * syncArticleKey = SHA-256(源 syncId + 类型标记 + 证据) 前 32 位；**类型标记进摘要**，因此
            「GUID 与链接恰好同串」「可靠与不可靠指纹」都得到不同的键；
          * 三个证据都缺失时返回 **null**（不编一个键让两台设备上的无关文章互相覆盖状态）；
          * syncPlaceholderKey 由「本机订阅 syncId + 远端键」派生；planRemoteStateArrival 区分「本机
            没有 → 建占位行」与「本机已有 → 只应用状态」；upgradedIdentityBasis 只允许 remote → 真实
            依据（**绝不反向**）；
          * alignFeeds 先按 syncId、再按**唯一的**规范化 URL 对齐，一条本机订阅只被对齐一次；
            aliasesToPersist 只在 URL 匹配时写别名；
      - 新增 core/domain/sync_store.dart（同步状态端口：基线/待同步变更/墓碑/别名/远端状态应用 +
        SyncBaseline/PendingChange/SyncTombstone/RemoteArticleState/RemoteStateOutcome 数据类）；
      - 新增 infrastructure/local/sync_store.dart（drift 实现）与 tables/sync_tables.dart（四张表）；
      - IdentityBasis 增加 remote（占位行的判定依据：**只有状态、没有正文**；不复用 fingerprint，否则
        「本机从未见过这篇」与「按标题+时间兜底匹配到」在数据上不可分辨）；
      - articles 增加 syncKey 列 + 条件唯一索引 ux_articles_sync_key（占位行靠它被后续抓取匹配上；
        可空且**不回填**）；
      - database.dart 升到 v16：建四张表与六条索引、加 articles.sync_key 与索引、写入 sync_state 的
        **单行种子**（基线 null / 修订 0 / 能力未探测）；**不回填**任何待同步变更或墓碑（升级前不存在
        「有改动等着同步」这个事实）；highestImplemented 同步到 16；
      - 新增 drift_schemas/drift_schema_v16.json 与 test/generated/schema_v16.dart；把 12 个既有迁移
        用例的 currentSchemaVersion 由 15 改为 16（只改数字，因此它们始终验证真实路径）；
      - 测试：新增 sync_projection_test（21 条：C 类纳入且与注册表一致、S 类与 D 类逐项排除、未注册编号
        不纳入、订阅/分组/文章字段的纳入与排除、投影里不出现凭据类字段名、T036 规则纳入、实体类别稳定）、
        sync_article_key_test（31 条：同输入同键/异源同 GUID 异键/类型标记参与摘要/判定顺序/指纹可靠度/
        无身份不给键/空白证据/键不含原文、占位键与升级方向、对齐的四条规则、别名正反查与去重、私密 URL
        三种情形与凭据引用区分）、migration_v15_to_v16_test（7 条：带旧数据升级零丢失、单行种子即诚实
        默认、不回填、新库建表与索引、四条 DDL 约束）、sync_store_test（18 条：基线默认与推进、**确认按
        修订号上界**（上传期间的新改动必须留下）、固定变更集合、同步成功后修订号不回退、墓碑幂等且不随成
        功清除、别名幂等、占位行的六种情形、同步表无凭据列）；
      - schema_snapshot_test 增加 v16 快照用例（新增实体清单 + 四张表的**逐列**断言 + 无凭据列）。
    数据迁移/删除/依赖变化：schema v15 → v16（四张新表 + 六条索引 + articles.sync_key 与一条条件唯一
      索引 + sync_state 单行种子）；**无删除**；无新增第三方依赖。
    环境：macOS 27.0 (26A428) / Apple M4 / 16 GiB / arm64；Flutter 3.47.0 / Dart 3.13.0；debug（macOS）
    检查（均为本机实际执行，命令 | 退出码 | 结论 | 证据）
      flutter pub get | 0 | PASS | Got dependencies
      dart run build_runner build | 0 | PASS | wrote 690 outputs
      dart run drift_dev schema dump lib/infrastructure/local/database.dart drift_schemas/ | 0 | PASS | 写出
        drift_schema_v16.json
      dart run drift_dev schema generate --data-classes --companions drift_schemas/ test/generated/ | 0 | PASS |
        写出 schema_v16.dart
      dart format lib test | 0 | PASS | 无格式差异
      flutter analyze | 0 | PASS | No issues found
      flutter test | 0 | PASS | 1863 通过 / 2 跳过 / 0 失败（相对 T040 的 1799 新增 64 条）
      flutter build macos --debug | 0 | PASS | build/macos/Build/Products/Debug/Flux.app（与 T042 同一次构建；本轮两轮改动一起构建）
    费用与秘密：**未发起任何真实网络调用，无费用产生**；新增的投影清单与测试逐条断言「S 类秘密不进投影」
      「订阅的 credentialRef 不同步」「同步四表没有凭据列」；凭据引用只带定位键（类别 + syncId），不含值
    遗留问题与未运行项：
      1) **缺失 REVIEW**：本轮交付的是数据出境边界（哪些东西会离开本机）与跨设备文章键的规则，其中
         「纳入/排除清单的取舍」「占位行用独立判定依据 remote 而不是复用 fingerprint」未经产品复核；
      2) **占位行 UI 尚未接线**：占位行已能落库并被后续抓取按同步键匹配，但列表/详情如何呈现「正文尚未
         同步」（而不是「正文为空」）属 T044/T045 的界面范围；
      3) **抓取补正文的接线未做**：upgradedIdentityBasis 与 syncPlaceholderKey 提供了判据，但把「刷新
         抓到的文章升级占位行」接进刷新管线属 T045 范围——本轮只交付规则与存储路径；
      4) **未做双设备实测**：全部断言在单库/纯函数上完成，「两台设备各自导入同一源后真的对齐」需要两台
         真实设备与真实 WebDAV（属 T044 的「两个隔离客户端真实 WebDAV 验证」）；
      5) **同步未接真实网络**：本轮不含任何网络调用（投影与键都是纯函数，存储在 SQLite 上），实际传输见
         T042；6) Android 工程仍未初始化；7) 本轮成果未 push 到远端。
    需求是否变化、维护者是否批准：未改变任何验收条件文字；只更新任务状态列（T041 → DONE，标注缺失
      REVIEW）、功能账本与 R041 记录。未删除任何功能或数据。
    提交/差异范围：提交 "T041: sync schema, stable article keys, C-D-S projection and placeholder states"；
      基线为 T040 的提交（a134c06）。未 push
    下一可执行任务及前置条件：T042（WebDAV 能力探测与不可变快照传输），前置 T041 已 DONE

### 7.1.36 轮次记录 R042（T042）

    轮次/日期：R042 / 2026-09-22
    任务 ID 与状态变化：T042（WebDAV 能力探测与不可变快照传输）TODO → DONE（缺失 REVIEW）
    相关决策/功能/SET 项：架构 5.2（验证强 ETag/If-Match、不可变快照 + 共享 manifest、条件发布、412
      重试 ≤ 3 轮、孤儿快照不是当前版本、缺条件写则降级只读拉取）、架构第 8 节（秘密不进日志/URL；不全局
      忽略 TLS；不可信输入在边界校验）、SET-070/071、手册 6.3「同步」节
    修改文件与主要行为：
      - 新增 core/domain/webdav.dart（**纯规则**，因此协议行为不需要真实服务器就能逐条钉住）：
          * evaluateConditionalWriteProbe：四步事实判定，顺序本身是设计——写不成功 → 无 ETag →
            If-Match 被拒 → 写入后 ETag 未变 → **过期 ETag 未被拒绝**，任一不满足即 readOnlyPull。
            最后一步是关键：忽略 If-Match 的服务器能通过前三步，只有在多端并发时才会静默互相覆盖；
          * snapshotFileName/snapshotContentHash：**内容寻址**命名（名字即内容），因此「manifest 指向
            的版本」与「快照内容」的关系可校验；versionForSnapshot 由哈希派生（同内容同版本）；
          * SyncManifest：指针字段（version/snapshotName/snapshotHash/parentVersion）与元数据字段
            （revision/deviceName/publishedAt）**分开**——后者不参与「谁更新」的判定（架构禁止仅比较
            设备墙钟）；decode 对缺字段/类型错/**更高协议版本**一律返回 null（不用默认值凑一个、不按旧
            形状解读新格式）；
          * isCurrentSnapshot/orphanSnapshots/canConfirmPublish：孤儿快照判据与「本地事务确认」的三件
            读回事实；
      - 新增 infrastructure/network/webdav_client.dart：PROPFIND（Depth 0/1）、GET、PUT（If-Match）、
        DELETE、MKCOL；密码**只进 Basic 认证头**（不进 URL/日志/错误消息）；响应体两道限长（声明长度 +
        流式计数）；失败路径也读干响应流以便复用连接；地址走 UrlGuardPolicy.configuredSource（用户显式
        配置的端点，允许内网）；412 翻译成 conflict 而不是抛错；MKCOL 的 405 视为成功（幂等）；
        XML：parseMultiStatus 先拒 DTD/ENTITY（与 feed_parser/opml 同一条防线，独立实现），再用事件流
        限深度 64/节点 20000，只取 resourcetype/getetag/getcontentlength/getlastmodified 与 href；
        normalizeHrefPath 把完整 URL、percent 编码与末尾斜杠归到同一路径；无 href 的 response 被跳过
        （不猜一个路径出来）；
      - 新增 infrastructure/network/webdav_snapshot_publisher.dart（发布编排，**不碰数据库**，因为架构要
        求网络调用不得放在长事务内）：读 manifest+ETag → 上传不可变快照（PROPFIND 判重，同名跳过）→
        **读回校验哈希** → 条件写 manifest（首次用 If-Match: *）→ **412 重读重试 ≤ 3 轮** → 读回校验
        manifest → 返回结论让上层在本地事务里确认；能力非 conditionalWrite 时**一个字节都不写**；快照已
        写而 manifest 未更新 → orphanSnapshot（不是当前版本，本机状态不变）；
      - 测试新增 test/infrastructure/network/fake_webdav_server.dart（**状态化**内存服务器：文件表 + ETag
        + If-Match 判定 + 412 + MKCOL/PROPFIND/GET/PUT/DELETE + 断网开关 + 请求日志，可被继承以模拟
        「忽略 If-Match」「每次 412」「读回被篡改」「manifest 被拒」四类服务器行为）与 webdav_test.dart
        （45 条，七组：能力探测六种判定 + 弱 ETag 识别、Multi-Status 解析与 XML 防护七条、请求往返十条
        （含密码不进 URL/日志、非 http 不发请求、限长）、发布协议五条（首次/父版本链/幂等跳过/读回校验
        失败不写 manifest）、条件写冲突五条（412 后第二轮成功且基于新基线、三次用尽放弃、忽略 If-Match 时
        零写入、unknown 能力 fail-closed）、孤儿快照四条、manifest 编解码七条）。
    数据迁移/删除/依赖变化：**无 schema 变更**；无删除；无新增第三方依赖（沿用 http 与 xml）
    环境：macOS 27.0 (26A428) / Apple M4 / 16 GiB / arm64；Flutter 3.47.0 / Dart 3.13.0；debug（macOS）
    检查（均为本机实际执行，命令 | 退出码 | 结论 | 证据）
      flutter pub get | 0 | PASS | Got dependencies
      dart run build_runner build | 0 | PASS | 无新表，与 T041 同一次生成
      dart format lib test | 0 | PASS | 无格式差异
      flutter analyze | 0 | PASS | No issues found
      flutter test | 0 | PASS | 1908 通过 / 2 跳过 / 0 失败（相对 T041 的 1863 新增 45 条）
      flutter build macos --debug | 0 | PASS | build/macos/Build/Products/Debug/Flux.app
    费用与秘密：**未发起任何真实网络调用，无费用产生**（全部走内存服务器）；新增用例逐条断言「密码不进
      URL/日志」「错误消息不含凭据」「DTD/ENTITY 被拒且不回显实体内容」
    遗留问题与未运行项：
      1) **缺失 REVIEW**：本轮交付的是同步协议的传输与条件发布规则，其中「降级只读拉取的判据（要求过期
         ETag 被拒）」与「孤儿快照的清理策略」未经产品复核；
      2) **未对真实 WebDAV 服务器做任何调用（NOT_RUN）**：无凭据（不允许伪造）。因此「某个真实服务在探测
         四步上的实际表现」**没有实测证据**；测试证明的是协议规则与客户端行为，不是服务器兼容性；
      3) **能力探测的采样未接线**：evaluateConditionalWriteProbe 是纯函数，探测结果已能写入 sync_state
         （T041 的 writeCapability），但「首次配置时自动跑那四步」的 UI/用例编排属 T044；
      4) **三方合并未做**：buildSnapshot 已按「每轮重试可产出不同内容」的形状留出接口（父版本传入），但
         合并语义本身是 T043 的范围；本任务只做**单机语义**（读取基线 → 上传 → 条件发布 → 确认）；
      5) **孤儿快照只判定不清理**：orphanSnapshots 给出判据，实际 GC 属 T047；
      6) **TLS/代理未做特别处理**：沿用既有 HTTP 客户端（不全局忽略证书校验）；7) Android 工程仍未
         初始化；8) 本轮成果未 push 到远端。
    需求是否变化、维护者是否批准：未改变任何验收条件文字；只更新任务状态列（T042 → DONE，标注缺失
      REVIEW）、功能账本与 R042 记录。未删除任何功能或数据。
    提交/差异范围：提交 "T042: WebDAV client with capability probe, immutable snapshots and conditional
      manifest"；基线为 T041 的提交。未 push
    下一可执行任务及前置条件：T043（三方合并/条件发布/墓碑/冲突），前置 T042 已 DONE


### 7.1.37 轮次记录 R043（T043）

    轮次/日期：R043 / 2026-09-22
    任务 ID 与状态变化：T043（三方合并/条件发布/墓碑/冲突）TODO → DONE（缺失 REVIEW）
    相关决策/功能/SET 项：架构 5.2（三方比较、字段级合并、同字段并发显示冲突、不得用三态逻辑 OR
      或仅比较设备墙钟、删除使用墓碑、降级只读拉取、上传期间新本地修改不丢失、远端发布成功后本机
      崩溃可幂等恢复）、架构 5.1（Feed / ArticleState）、SET-070/071/075、手册 6.3「同步」节
    修改文件与主要行为：
      - 新增 core/domain/sync_snapshot.dart（**跨设备交换的唯一内容形态**）：
          * 规范编码是**确定性**的（实体按 kind→key 排序、字段按名排序、列表顺序保留）：同一份
            内容永远编出同一串字节。这是内容寻址快照（T042：名字即内容）与「按版本幂等确认」
            的共同前提——编码一抖，同一份内容就有两个哈希，两条规则同时失效；
          * **删除是独立的一节**（SyncDeletion，带 keepFavorites 这项操作元数据）：删除不能被
            表达成「字段变成空」，否则合并会把缺失当成一次编辑；
          * **缺失 ≠ 删除**：某个键在远端快照里没出现只说明远端没报告它；
          * decode 对结构问题一律返回 null（不按旧形状解读、不用默认值凑一份）；
      - 新增 core/domain/three_way_merge.dart（**纯函数**，三个快照进、一个结果出）：
          * **签名里没有任何时间参数**，因此「按设备墙钟挑一方」无法被后来者顺手加上而不改签名；
          * 规则：双方同值保留 → 仅一方变更取该方（判据是与共同基线比较）→ 其余为冲突；
          * **同字段并发冲突不自动取胜方、不做 OR**：两个候选值原样返回（三态与收藏都不存在
            一个「合并后」的中间值）；用户选版后由 mergeWithChoices/applyConflictPolicy 形成新版本；
          * **缺失字段按「与基线相同」处理**（远端只报名字不会清掉本机的排列权重），
            显式 null 才算一次真实变更；
          * 删除：**本机墓碑优先于远端复活**（appliedDeletions 记录挡下了哪些），
            **远端破坏性删除只登记不应用**（pendingRemoteDeletions，合并结果里也不含该实体）；
      - 新增 core/domain/sync_transport.dart（传输端口 + SyncCredentials：密码只在**调用实参**
        里出现一次，toString 已脱敏）；新增 infrastructure/network/webdav_transport.dart
        （把 T042 的状态翻译成中性结论：412 → conflict、404 → notFound、密码每次从实参取）；
      - 新增 core/domain/sync_local_store.dart + infrastructure/local/sync_local_store.dart
        （本机内容快照的读写）：读出去哪些字段由 **T041 的投影**决定（本文件没有第二份「哪些
        字段敏感」的判断）；写回前逐字段查「该字段是否在本轮修订之后又被改过」（
        sync_pending_changes.revision > confirmedRevision），命中则跳过并记入
        preservedLocalFields——**上传期间的新修改必须留下**这条验收就落在这里；
        **写回不删任何业务行**（跨设备删除应用属 T045），占位行仍是「状态落库、正文为空、
        identityBased=remote」；本机已有另一条订阅占着同一规范化地址时**跳过该条而不是硬插**
        （硬插会撞唯一索引并让整批合并回滚，一条位置重复的订阅会连带文章状态都写不进去）；
      - 新增 core/domain/sync_first_merge.dart（首次合并预览：远端/本机项数、差异处数、
        待确认删除数、推荐策略固定为 preferLocal = **不覆盖本地**）；
      - 新增 infrastructure/local/sync_status_reader.dart（本机状态读取，**不发网络请求**）；
      - 新增 features/sync/application/sync_engine.dart（编排，按架构原话的顺序）：
        读基线 → 读本机快照 → 读远端 manifest + 快照 → 三方合并 → 上传唯一快照 → **读回校验
        哈希** → 条件写 manifest → **412 重读重合并重试 ≤3 轮** → **本地事务确认**；
        确认按修订号上界；412 之后基于**新读到的**父版本重合并；3 轮用尽 → waitingRetry 且
        远端 manifest 从未被本机改写；能力非 conditionalWrite 时**一个字节都不写**（只读拉取，
        返回 pulledSnapshot 供显式导入）；**崩溃恢复幂等**：判据是「远端当前版本的内容哈希 ==
        本机当前内容」（版本号由内容派生，因此这是精确判据），命中则只补一次本地确认；
        webdav.dart 增加 snapshotNameForVersion（版本 → 文件名是一次前缀匹配，避免在本地
        再存一份历史快照）；
      - 测试：新增 test/core/sync_snapshot_test.dart（13 条：确定性编码、列表顺序不被规范化、
        往返、解析防御、缺失与 null 可区分、版本↔文件名）、three_way_merge_test.dart（24 条：
        字段级合并、双向变更、三态不可 OR、收藏、时钟偏移无影响、并集、墓碑优先、远端删除只登记、
        选版、缺失语义）、test/features/sync/sync_engine_test.dart（14 条：首次发布、两轮 412 收敛
        与父链、三轮用尽不改远端、read↔later 冲突不发布、选版后形成新版本、取消收藏不复活、
        墓碑防复活、远端删除不应用、上传期间新修改保留、崩溃恢复幂等、降级只读零写入、
        能力未探测 fail-closed、时钟偏移一年仍只由基线决定）、sync_local_store_test.dart
        （13 条：投影清单出境、无同步键不参与、墓碑出境、并集写回、不删除、位置重复跳过不整批回滚、
        只改状态不动正文、占位行、修订号上界、必访站整体替换、幽灵设置不写入）。
    数据迁移/删除/依赖变化：**无 schema 变更**；无删除；无新增第三方依赖
    环境：macOS 27.0 (26A428) / Apple M4 / 16 GiB / arm64；Flutter 3.47.0 / Dart 3.13.0；debug（macOS）
    检查（均为本机实际执行，命令 | 退出码 | 结论 | 证据）
      flutter pub get | 0 | PASS | Got dependencies
      dart run build_runner build | 0 | PASS | wrote 718 outputs
      dart format lib test | 0 | PASS | 无格式差异
      flutter analyze | 0 | PASS | No issues found
      flutter test | 0 | PASS | 1972 通过 / 2 跳过 / 0 失败（相对 T042 的 1908 新增 64 条）
      flutter build macos --debug | 0 | PASS | build/macos/Build/Products/Debug/Flux.app
    费用与秘密：**未发起任何真实网络调用，无费用产生**（全部走内存服务器与内存库）；新增用例
      断言「密码不进快照」「快照里没有 credentialRef/password 字样」「SyncCredentials 的
      toString 已脱敏」
    遗留问题与未运行项：
      1) **缺失 REVIEW**：本轮交付的合并规则与冲突呈现口径（默认保留本机、选版后形成新版本）
        未经产品复核；
      2) **未对真实 WebDAV 服务器做任何调用（NOT_RUN）**：无凭据（不允许伪造）。因此「真实服务
         是否按强 ETag/If-Match 行为」没有实测证据；测试证明的是规则与编排，不是服务器兼容性；
      3) **远端破坏性删除的应用未做**（只登记待确认）：应用属 T045 的跨设备删除集成；
      4) **孤儿快照只判定不清理**（判据在 T042，GC 属 T047）；
      5) **占位行的界面呈现**（「正文尚未同步」）属 T044/T045；
      6) Android 工程仍未初始化；7) 本轮成果未 push 到远端。
    需求是否变化、维护者是否批准：未改变任何验收条件文字；只更新任务状态列（T043 → DONE，
      标注缺失 REVIEW）与 R043 记录。未删除任何功能或数据。
    提交/差异范围：提交 "T043: three-way merge, conditional publish, tombstones and crash-safe
      confirmation"（172f1d2）；基线为 T042 的提交（a8d0153）。未 push
    下一可执行任务及前置条件：T044（同步设置页、触发排队与首次合并），前置 T043 已 DONE

### 7.1.38 轮次记录 R044（T044）

    轮次/日期：R044 / 2026-09-22
    任务 ID 与状态变化：T044（同步设置页、触发排队/首次合并）TODO → DONE（缺失 REVIEW；真实
      WebDAV 双客户端验证按 6.3 记为 **NOT_RUN**）
    相关决策/功能/SET 项：SET-070–075（WebDAV URL/用户/密码/远端目录/设备名、同步启用与三种触发、
      自动间隔与仅手动、同步范围、首次同步与冲突处理）、架构 5.2（首次预览不覆盖、同字段并发人工
      选版、降级只读拉取、未批准不自动清除本地内容）、架构第 8 节（密码只进钥匙串）、手册 6.3
      「同步」节、D-08（调度是本机项）
    修改文件与主要行为：
      - 新增 features/sync/application/sync_settings.dart（设置读取与 URL 校验）：
          * 默认回退**方向保守**：读不到时同步**关着**（syncOnStart 开、防抖 5 秒、间隔 30 分钟、
            首次预览开），因此一次读失败不会让一台从没配置过的设备突然开始上传；越界值夹到合法区间；
          * validateSyncUrl/parseSyncEndpoint 复用客户端**同一个守卫**（configuredSource 策略：
            允许内网，拒绝非 http/https 与缺主机名），且**区分四种原因**（未填写/格式错/协议不符/
            缺主机名）——用户要改的地方不同；
          * SyncConnectionProbe/SyncConnectionProber 端口：把「探测」定义成一个**只有读**的动作；
      - 新增 features/sync/application/sync_manager.dart（SyncManager + SyncStatusSnapshot）：
          * 四个触发来源（启动/定时/变更防抖/手动）走**唯一入口 request**；运行中再次触发只记一个
            **待跑标记**（不排队成 N 次），结束后**串行续跑**（同机不并发）；
          * 防抖**每次重新计时**；manualOnly 不起定时器；未配置或关闭时**一个请求都不发**；
          * 能力**只探测一次**（结论记在状态里复用）；
          * **首次合并预览**：没有共同基线且远端已有内容时先预览（只读本机 + 远端算差异），
            默认策略 **preferLocal（不覆盖本地）**；用户确认后才建立基线并发布；
          * **冲突选版**：manual 下不发布，冲突列表进状态；resolveConflicts 之后形成新版本；
            **成功发布后状态里的冲突被清掉**（引擎仍在结果里保留「这次发生过哪条冲突」这个事实，
            但界面读的是「还有多少事要用户做」）；
          * 状态是**单一来源**：管理器唯一写出 SyncStatusSnapshot，界面读它；
      - 新增 features/sync/application/sync_providers.dart（管理器/密码/探测端口 + 状态发布点）；
      - 新增 features/sync/presentation/sync_settings_page.dart 与 sync_status_text.dart
        （设置页 + 文案映射）：服务器四项（SET-070，密码不回显、写完立刻清空输入框）、
        **测试连接只读**（界面上写明「不会写入或修改远端任何内容」）、触发四项（SET-072/073）、
        同步范围（清单**来自 T041 的投影**，只展示数量与说明，不手抄）、当前状态、手动同步、
        降级提示条、首次合并卡片（默认按钮是「合并并保留本机内容」）与冲突卡片（字段/基线值/
        本机值/远端值/选择控件）；
      - 新增 infrastructure/network/webdav_capability_probe.dart（**做那四个实验**并把事实交给
        T042 的纯函数判定；只动自己的探测文件、探测完删掉；「忽略 If-Match 的服务器」被判成
        readOnlyPull）、webdav_connection_prober.dart（只读探测：只 PROPFIND + GET，
        四种结论可区分）、infrastructure/platform/webdav_secret_store.dart（SET-071 → Keychain，
        类别前缀 webdav，与 feed-auth/ai-provider/search-service 互不覆盖）、
        infrastructure/local/degraded_sync_local_store.dart（降级启动：读失败、写失败，不假装可用）、
        sync_status_reader.dart 与 core/domain/sync_status.dart（本机状态读取，不发网络请求）；
      - 接线：lib/app/app_providers.dart 装配四件（密码端口/只读探测/管理器/状态发布），并在启动时
        调一次 onLaunch（SET-072「启动同步」的落点，与 T040 定时总结同一口径）；设置页新增
        「同步与备份」入口（真导航行，不是禁用占位项）；l10n 新增 78 条中英文案；
      - 测试：新增 sync_settings_test.dart（9 条：默认回退的方向、坏值不猜、已配置读取、远端目录
        回退、无秘密、URL 四种结论、两个入口结论一致）、sync_manager_test.dart（12 条：启动同步与
        未配置零请求、关闭时手动也不发、同机不并发（可阻塞传输构造确定窗口）、防抖、manualOnly、
        首次预览不写、确认后建立基线且本机内容仍在、关闭预览直接合并、冲突不发布→选版→新版本、
        preferRemote、降级只读、能力只探测一次）、sync_settings_page_test.dart（6 条：默认关闭与
        按钮禁用、SET-070 落库、非法地址两种说明、测试连接只读且有结论、密码不回显、
        降级条由状态驱动）、test/infrastructure/network/sync_probe_test.dart（10 条：能力探测四类
        服务器判定与「只动自己的探测文件」、只读探测的方法集合恰为 {PROPFIND, GET}、
        目录缺失/认证失败/地址非法各自的结论与零请求）、test/app/settings_page_test.dart 新增
        1 条（同步入口可进入同步页且范围清单来自投影）；同步更新主题 golden（设置页多了一个真入口）
        与 l10n 条目数（799 → 877）。
    数据迁移/删除/依赖变化：**无 schema 变更**；无删除；无新增第三方依赖
    环境：macOS 27.0 (26A428) / Apple M4 / 16 GiB / arm64；Flutter 3.47.0 / Dart 3.13.0；debug（macOS）
    检查（均为本机实际执行，命令 | 退出码 | 结论 | 证据）
      flutter pub get | 0 | PASS | Got dependencies
      dart run build_runner build | 0 | PASS | wrote 752 outputs
      dart format lib test | 0 | PASS | 无格式差异
      flutter analyze | 0 | PASS | No issues found
      flutter test | 0 | PASS | 2010 通过 / 2 跳过 / 0 失败（相对 T043 的 1972 新增 38 条）
      flutter build macos --debug | 0 | PASS | build/macos/Build/Products/Debug/Flux.app
    费用与秘密：**未发起任何真实网络调用、无费用产生**（能力探测与只读探测全部走内存服务器）；
      新增用例断言「密码不回显」「页面上不出现密码本身」「测试连接的方法集合恰为
      {PROPFIND, GET}」「地址非法时零请求」「同步范围清单来自投影」
    遗留问题与未运行项：
      1) **真实 WebDAV 双客户端验证 NOT_RUN**：手册 6.3 的「两个隔离客户端真实 WebDAV」需要维护者
         提供凭据（地址/用户名/密码）后执行；无凭据不伪造。**Mock 双客户端测试（内存服务器 + 两个
         隔离内存库）是本轮提交的 fixture 证据**，它证明的是规则与编排，不是某个真实服务的兼容性；
      2) **缺失 REVIEW**：首次合并默认「不覆盖本地」与冲突默认「选本机」这两个默认值未经产品复核；
      3) **远端破坏性删除的应用仍未做**（界面明说「本机数据尚未清除」，不放点了没用的按钮）：
         应用属 T045；
      4) **T046 备份恢复未做**：设置页入口文案里提到的「明文备份」仍是后续任务；
      5) macOS 没有后台执行的保证（与 T040 同一口径）：应用退出后不会继续同步，界面与文档如实说明；
      6) Android 工程仍未初始化；7) 本轮成果未 push 到远端。
    需求是否变化、维护者是否批准：未改变任何验收条件文字；只更新任务状态列（T044 → DONE，标注
      真实 WebDAV 验证 NOT_RUN）、功能账本与 R044 记录。未删除任何功能或数据。
    提交/差异范围：提交 "T044: sync settings, trigger queue, first-merge preview and conflict
      resolution UI"；基线为 T043 的提交。未 push
    下一可执行任务及前置条件：T045（删除与收藏保留跨设备集成），前置 T018/T043/T044 均 DONE；
      T046（明文完整备份与安全恢复）同样已无前置阻塞
...
### 7.1.39 轮次记录 R045（T045）

    轮次/日期：R045 / 2026-09-22
    任务 ID 与状态变化：T045（删除与收藏保留跨设备集成）TODO → DONE（缺失 REVIEW；真实 WebDAV
      双客户端验证沿用 R044 的 **NOT_RUN**）
    相关决策/功能/SET 项：SET-081（删除订阅每次询问、默认保留收藏）、架构 5.2（删除使用墓碑且首发
      不自动清除、删除操作元数据随删除传播、未批准不自动清除本地内容、远端状态到达落占位行、
      抓取补身份只允许 remote → 真实依据）、架构 4.1（删除订阅两个分支、收藏脱离源并冻结来源快照）、
      架构 5.3（清理规则）、手册 6.3「删除」与「同步」两节
    修改文件与主要行为：
      - 新增 core/domain/remote_deletion.dart（**纯规则**）：
          * RemoteDeletionTarget 与 remoteDeletionTargetOf：由快照里的实体类别判定目标（订阅/分组/
            不认识），**未知类别一律 unsupported**——猜一个目标去删是不可撤销的错误，而它的表现是
            「另一条订阅消失了」；
          * RemoteDeletionImpact：一条远端删除在本机的影响范围（订阅名/文章数/收藏数/later 数），
            可应用的构造入口只接受 T018 的 FeedDeletionPreview/GroupDeletionPreview（**数字只有一套
            算法**）；defaultKeepFavorites 沿用远端当时的选择（否则「远端说有 3 篇收藏留下」与
            「本机删完只剩 0 篇」会对不上）；
          * RemoteDeletionApplication：一次应用的结果（含本机这次的选择）；
          * isFeedTombstoned / shouldSkipRefreshForTombstone：调度侧的墓碑判据；
          * FeedTombstoneReader 端口与 NoopFeedTombstoneReader（默认返回**空集合**：刷新是只读的，
            漏接线必须退化成「照常刷新」，不能让一个与刷新无关的接线疏忽停摆全部刷新）。
      - 新增 features/sync/application/remote_deletion_use_case.dart：
          * preview/previewAll：**只读**算出影响范围（分组按组内订阅逐条累加，复用单源预览，因此与
            「逐个删源」的数字一致）；本机没对齐到那条删除时返回不可应用与稳定原因名
            （feedNotAligned/groupNotAligned/unsupportedEntityKind），**不猜目标**；
          * applyConfirmed：拒绝不可应用的目标（类型化失败，不假成功）；订阅走 catalog.deleteFeed、
            分组按操作元数据选分支（有 keepFavorites 走 deleteFeeds，没有走 moveToUncategorized），
            全部复用 T018 事务（保留收藏、非收藏含 later 清理、收藏脱离源并冻结来源快照）；
          * 应用动作写诊断（tag 为 sync.remoteDeletion），只记数量与显示名，不记正文。
      - 新增 infrastructure/local/deletion_sync_recorder.dart：
          * recordLocalDeletionFact：**必须在事务内**调用，写墓碑 + 待同步标记 + 推进本地修订号
            （修订号由本机分配，不给调用方传数字）；重复删除幂等且不刷新删除时间；
          * forgetLocalDeletionTombstone：本机**显式重新添加**同一源时忘掉旧墓碑。订阅 syncId 是
            规范化地址的摘要，重新添加必然得到同一个 syncId——不忘记会让那个源永远跳过刷新且没有
            任何线索；这**不**违反「已删除条目不复活」（那一条约束的是别的设备的旧快照）。
      - feed_catalog_store.dart：deleteFeed 与删除分组的**两个分支**都在同一事务内调用记录器
            （分组行已删就该留墓碑）；createFeed 先忘掉同 syncId 的旧墓碑。
      - core/domain/article_catalog.dart：ArticleListEntry 增加 identityBasis 与 awaitingBodySync
            （「正文尚未同步」是独立状态，与「正文为空」「仅摘要」不同）。
      - article_store.dart（占位行升级）：在三条身份规则之后加**按同步键匹配**这第四条路径——
            候选键两个（本机这次算出的键、以及由它派生的占位键），命中且 identityBasis 为 remote 时
            才接管；补上标题/正文/身份依据与**同步键**（从远端派生的占位键升到本机算出的键），
            readingState 与 favorite 一律不动。真实身份的行不会被这条路径改写。
      - refresh_scheduler.dart：派发前按墓碑 syncId 集合过滤，跳过的源进 RefreshRunReport.skipped
            （reason 为 tombstoned），**不是静默丢弃**；墓碑读取失败或未接线时按「没有墓碑」放行。
      - infrastructure/local/feed_tombstone_reader.dart（窄端口实现）+ feed_ports.dart 的
            feedTombstoneReaderProvider + app_providers.dart 接线。
      - sync_local_store.dart：读快照时把 T018 deletion_events 的 keepFavorites 按 syncId 关联到
            墓碑上，因此**保留收藏的选择随快照离机**（架构 5.2 的删除操作元数据）。
      - 界面：同步设置页的远端删除卡片从「只读说明」改为**确认区**（先预览影响范围 + 保留收藏勾选
            + 确认按钮；不可应用项不给按钮并说明原因；确认后回执可见）；文章卡片与详情页显示
            「正文尚未同步」（列表徽标 + 详情页横幅与徽标）。
      - l10n：新增 9 条（确认按钮、保留收藏选项、影响范围四个数字、无文章、不可应用、应用回执、
            预览失败、占位行说明与徽标），并把 T044 的 syncRemoteDeletionNote 从「属后续任务」改为
            当前口径；l10n_test 的条目数 877 → 886。
    用例与证据：新增 test/features/sync/remote_deletion_test.dart（20 条）。断言的是**数据**而不是
      「调用了某个方法」：预览后文章数与订阅数都不变（未确认不动数据）；确认后非收藏含 later 全清理、
      收藏留下且 feedId 置空、feedTitle 冻结为来源快照；不保留分支全清；没对齐的删除拒绝执行且一行
      不动；未知类别不猜；分组影响范围逐条累加且两个分支各自正确；本机删除后墓碑 + 待同步标记 +
      修订号推进、快照里的删除事实带 keepFavorites；应用远端删除之后本机也产生墓碑并写下这次的选择；
      重新添加同一源后墓碑消失；墓碑源在派发前被拦下（请求列表里没有它）且进 skipped；墓碑读取失败
      与未接线时照常刷新；占位行按同步键被升级（一行不变多、正文与身份补上、**later 与收藏保留**、
      同步键升到本机算出的键）；无证据时两行不合并；真实身份行不被改写；缺单行同步状态时记录器抛错。
      组件层另加 3 条：同步设置页的远端删除卡片（影响范围可见 + 默认勾选沿用远端选择 + 未确认时
      3 篇文章与 1 条订阅都在 + 点确认后只剩收藏那 1 篇且脱离源 + 回执可见）、文章卡片与详情页的
      占位行状态（与「仅摘要」分别可见，正文仍为空、阅读状态不被打开动作改写）。全库 2033 tests 绿。
    遗留问题与未运行项：
      1) **真实 WebDAV 双客户端验证 NOT_RUN**（延续 R044）：无凭据不伪造；本轮的跨设备删除用
         「同一份规则 + 真实内存库」验证，双设备 fixture 仍来自 R044 的 Mock 服务器；
      2) **缺失 REVIEW**：远端删除确认框的默认勾选沿用远端选择（而不是一律默认保留）未经产品复核；
         分组删除在「操作元数据缺失」时按移动分支处理这一条同样待复核；
      3) **T046 备份恢复未做**：设置页入口提到的「明文备份」仍是后续任务；
      4) 分组级墓碑只覆盖「分组行被删除」；分组内订阅的墓碑仍由删除订阅的路径逐条产生（这是刻意的：
         移动分支不删订阅，因此不能给它们写墓碑）；
      5) 其余同 R044（macOS 无后台执行保证、Android 未初始化、未 push）。
    需求是否变化、维护者是否批准：未改变任何验收条件文字；只更新任务状态列（T045 → DONE）、
      功能账本、R045 记录与 M3 出口口径。未删除任何功能或数据。
    提交/差异范围：提交 "T045: cross-device deletion integration with tombstone-aware refresh and
      placeholder upgrade"；基线为 T044 的提交（e0a506c）。未 push
    下一可执行任务及前置条件：T046（明文完整备份与安全恢复），前置 T009/T010/T041 均 DONE


### 7.1.40 轮次记录 R046（T046）

    轮次/日期：R046 / 2026-09-22
    任务 ID 与状态变化：T046（明文完整备份与安全恢复）TODO → DONE（缺失 REVIEW）；**M3 出口条件
      达成（T041–T046 全部 DONE）**
    相关决策/功能/SET 项：SET-076（导出明文备份/是否含媒体/恢复）、架构 5.3 的备份与恢复段、
      架构第 8 节（凭据不落库、无明文回退）、手册 6.3「恢复」节
    修改文件与主要行为：
      - 新增 core/domain/backup.dart（**纯规则**）：
          * backupFormatVersion（包结构版本，独立于 SET 编号）与条目白名单 backupEntryAllowed：
            只有「快照 / manifest.json / media 前缀」允许进包；
          * isSafeBackupEntryName 拒绝绝对路径、上跳、反斜杠、盘符（含 UNC）、空段与点号；
          * backupSchemaIsRestorable：备份 schema 不高于当前才可恢复；
          * BackupEntryRecord / BackupManifest：清单编解码确定性（条目排序、缩进固定），结构问题、
            更高包版本、白名单外条目一律返回 **null**，不用默认值凑一份清单；
          * BackupFailureKind（13 类稳定原因名）与 isBackupSecurityFailure（安全类可单独识别）；
          * validateBackupArchive：逐项校验（名字安全 → 白名单 → 清单 → schema → 快照存在 →
            大小与 SHA-256 → 清单之外无多余条目），全部先于任何写操作；
          * backupEntryRecordsOf：导出侧由条目生成清单记录（大小 + SHA-256）。
      - 新增 core/archive/zip_archive.dart（**自实现 ZIP**，不新增依赖）：
          * zipEncode 用 dart:io 的 ZLibCodec(raw: true)（DEFLATE），小条目自动退回 stored；文件名
            置 UTF-8 标志位（不设它会让中文媒体名在别的解压器里变乱码）；
          * zipDecode 接受 stored 与 deflate，拒绝 Zip64 哨兵值、未知方法、CRC 不符、越界偏移，
            任何结构问题返回 null（恢复据此报「包损坏」而不是解开一半）。
      - 新增 features/sync/application/backup_ports.dart：BackupContentSource（一致性快照/版本/
            计数/媒体清单）与 BackupRestoreTarget（写新目录 + 校验可用）；**两个端口的签名里都没有
            任何凭据参数**，因此「备份顺手带上密码」在这条路径上不可表达。
      - 新增 features/sync/application/backup_use_case.dart：export（快照 → 清单 → ZIP → 保存）、
            inspectBackup（选文件 + 只读预检，返回 BackupInspection：预览 + **当次字节**）、
            restore（校验 → 写新目录 → 校验新库）、validateArchiveBytes；恢复必须用预检时读到的
            那一份字节（两次点击之间换掉文件时，被恢复的必须仍是用户确认过的那一份）。
      - 新增 features/sync/application/backup_providers.dart：backupUseCaseProvider 与
            backupNewDirectoryProvider（**默认抛错**：空实现会让「导出成功」变成磁盘上什么都没有；
            规划器返回空串会让恢复写到当前数据目录，破坏「失败不动原库」）。
      - 新增 infrastructure/local/backup_content_source.dart：
          * DriftBackupContentSource.snapshotDatabase 用 VACUUM INTO 取一致性快照（临时文件写在
            数据目录，finally 里删除）；schemaVersion 读 PRAGMA user_version；contentCounts 用
            聚合 SQL；readMediaCache 只取 .img/.meta 并**排除 .part**，结果排序使同内容同包；
          * FileBackupRestoreTarget.writeRestoredData 拒绝非空目标目录，只写快照与媒体；条目名再经
            p.basename 落盘（保证最终路径一定在 media 目录之下）；
          * verifyRestoredData 真的 openAppDatabase 一次并核对 user_version（写入完成不等于恢复成功）。
      - 新增 infrastructure/local/degraded_backup_source.dart：降级启动下所有读取**明确失败**，
            而不是返回空快照（空快照会让用户以为数据备份好了）。
      - core/storage_layout.dart（新）：把数据目录/数据库/日志/媒体四个名字从 lib/app 提到 core
            （备份要写这些名字，而 infrastructure import lib/app 会成环），app_bootstrap 保留同名符号。
      - file_access.dart + file_selector_access.dart：新增二进制入口 pickArchiveToRead / saveBytes
            （ZIP 用文本读会被替换字符改写，哈希校验必然失败），PickedFile 增加 bytes 字段。
      - 界面：新增 features/sync/presentation/backup_section.dart（备份与恢复小节），并接进同步设置页；
            风险告知常驻可见 + 导出前确认 + 预检信息 + 二次确认 + 进度 + 失败分类展示。
      - 组合根：备份用例与恢复目录规划器接线（数据目录不可用时抛错），并作为 bootstrapOverrides 的
            参数以支持测试注入（Riverpod 禁止同一 Provider 重复覆盖）。
      - l10n：新增 18 条备份文案；l10n_test 的条目数 886 → 904。
    用例与证据：新增 test/features/sync/backup_test.dart（24 条）与 test/core/zip_archive_test.dart
      （6 条：自写自读、stored 回退、**与 Python zipfile 双向互操作**、截断/垃圾/篡改、Zip64 拒绝），
      组件层 test/features/sync/backup_section_test.dart（6 条）。断言的是**数据与文件**：清单里每个
      文件的 SHA-256 与内容一致；包内不出现任何凭据形态（逐条断言 webdav: / ai-provider: /
      search-provider: / feed-auth: / SET-071 / SET-027）；含媒体开关的分支各自落对；导出后数据目录里
      不残留临时快照；**WAL 一致性**用「写入 50 篇后立即导出」构造，恢复后逐行核对 52 篇（复制活动库
      文件会丢掉它们）；路径穿越的包被拒后原库与目录**完全没变**；损坏 ZIP / 错包版本 / schema 更新 /
      哈希不符 / 大小不符 / 清单条目缺失 / 目标目录非空各自失败且原库不动；恢复后订阅/正文/收藏/later
      全在且真能打开；无效数据库文件被 verify 拦下；清单编码确定性（含条目顺序无关）。全库 2069 tests 绿。
    遗留问题与未运行项：
      1) **缺失 REVIEW**：备份恢复的「二次确认」与「恢复到并列目录、重启后生效」这一交互未经产品
         复核（另一个可选口径是恢复后立刻切换并重启应用）；
      2) **切换步骤未自动化**：本任务交付的是「恢复到新目录 + 校验通过 + 返回新目录路径」，而把运行时
         数据目录指过去需要重启应用（架构 5.3 的「正式切换前再次确认」已由二次确认覆盖）；自动化切换
         属 T048 的故障恢复编排范围；
      3) **诊断日志不进备份**（清单白名单里没有它）：日志含运行期结构信息，与「用户数据备份」是两件事，
         诊断导出属 T048；
      4) **媒体缓存恢复后不解析校验**（只按清单校验它自己的大小与哈希）：解析会引入图像解码依赖，
         而损坏的缓存本来就是可再生内容（清缓存即可）；
      5) 真实 WebDAV 双客户端验证仍为 NOT_RUN（无凭据，延续 R044）；其余同 R045（macOS 无后台执行
         保证、Android 未初始化、未 push）。
    需求是否变化、维护者是否批准：未改变任何验收条件文字；只更新任务状态列（T046 → DONE）、功能账本、
      R046 记录、M3 出口说明。未删除任何功能或数据。
    提交/差异范围：提交 "T046: plaintext backup export and validated restore to fresh directory"；
      基线为 T045 的提交（6437dc8）。未 push
    下一可执行任务及前置条件：T047（存储分类、清理策略与缓存限额），前置 T018/T021/T030/T038 均 DONE；
      T048 的前置里 T046 已就绪

### 7.1.41 轮次记录 R047（T047）

    轮次/日期：R047 / 2026-09-22
    任务 ID 与状态变化：T047（存储分类、清理策略与缓存限额）TODO → DONE（**缺失 REVIEW**：本轮
      定义了「哪些内容可以被自动删除」这条不可逆边界，并给 AI 结果缓存加了容量上限与淘汰策略，
      两者都属复核适用范围）
    相关决策/功能/SET 项：SET-077（媒体/文章/总结自动清理开关与天数）、SET-078（自动清理包含
      收藏/稍后再读）、SET-079（清理预览/一键清缓存/占用刷新）、SET-080（媒体缓存上限）、
      架构 5.3 的清理段（「清缓存只删可再生内容，不删付费 AI 结果、状态、订阅、prompt 或秘密」
      「收藏默认保护，later 默认保护」「只释放正文时保留身份和状态，防止刷新把文章复活成未读」
      「总结引用用最小快照」「用户彻底删除时列出关联摘要/引用/缓存并清理，不能留下隐藏副本」）、
      架构第 8 节（凭据不进日志/诊断/备份）、T030 遗留（缓存无上限与淘汰）、T042 遗留（孤儿快照
      只判定不清理）
    修改文件与主要行为：
      - 新增 core/domain/storage_cleanup.dart（**纯规则**）：
          * StorageCategory 五类（媒体缓存/文章正文/新闻总结/其他缓存/设置与状态数据库）与
            StorageUsageReport（含**测量时刻**——架构 5.3 把「测量时间」与分类并列提出，一个没有
            时间的数字无法让用户判断它是否还对得上当前状态）；usageOf 缺类返回 0 而不是抛错；
          * AutoCleanupPolicy：三个独立开关 + 三个天数 + 两个保护开关，**默认全关**（默认开启会让
            一次升级后的首个启动就悄悄删掉用户的正文）；fromSettings 对单个字段的类型错/越界**逐项**
            回退默认值（一个坏字段不该让另外两个开关的意图一起失效）；copyWith 供界面整块写复合项；
          * cleanupCutoff（按**时长**而不是本地日期减天数：按日期算会在时区/夏令时变化时多留或少留
            一整天）与 isCleanupExpired（严格早于；**没有时间戳的行不猜**，返回 false）；
          * protectsFromAutoCleanup：收藏与 later 的默认保护，两个开关**互相独立**；
          * CleanupImpact：预览与执行**共用同一形状**，因此「预览说释放 3 MiB、执行后也是 3 MiB」
            可以被断言；protectedArticles（被保护而跳过的篇数）必须在预览里可见；
          * orphanSnapshotsToCollect：在 T042 的「不是当前版本」之上再加**时间**维度与引用集合，
            刚上传（manifest 还没写）或拿不到 getlastmodified 的一律不回收；
          * ArticlePurgeImpact：彻底删除的关联范围（引用/译文/会话/缓存媒体/新闻引用）。
      - 新增 core/domain/ai_cache_bounds.dart（**纯规则**）：AiCacheLimits（默认 200 条 / 128 MiB，
            clamped 对 0/负数**回退默认值**而不是夹到 1——夹到 1 会让一个坏值变成「几乎清空缓存」，
            那是最贵的后果）、AiCacheEntryFact（只带判定需要的元数据，**不带文本**）、
            planAiCacheEviction（两个上限都要满足；按**最近使用**升序淘汰；同一时刻按键排序保证
            顺序确定；本次刚写入的条目不被删；从未被读过的按写入时刻参与淘汰）、utf8ByteLength。
      - 新增 features/settings/application/cleanup_ports.dart：StorageCleanupStore（数据库侧：
            分类统计、正文释放的枚举与执行、总结清理、缓存与失败草稿的统计与清空、彻底删除的
            关联枚举与执行）、MediaCachePort（文件系统侧：统计/磁盘占用/到期枚举与删除/清空/上限）、
            SnapshotGcPort（列远端快照与时间、删一个）；**三个端口的签名里都没有秘密**，也没有
            「清掉一切」的入口——可删的东西按类别各有方法。
      - 新增 features/settings/application/cleanup_service.dart：measure（把媒体与数据库两个来源
            合并，媒体失败**不导致整页失败**）、previewClearRegenerable/clearRegenerable（**先读后删**，
            回执里的释放量是真实数字）、previewAutoCleanup（全关时**不访问存储**直接返回空）、
            runAutoCleanup（三类各自独立，某步失败不阻止其余——它们之间本来没有依赖）、
            previewArticlePurge/purgeArticle、collectOrphanSnapshots（未配置同步返回 Ok(null) 而
            不是错误；dryRun 只报告；单个删除失败不中断整轮）。
      - 新增 infrastructure/local/storage_cleanup_store.dart（drift 实现）：五条纪律写进文件头；
            **releaseArticleBodies 的 UPDATE 只写正文列**（body/body_hash/extracted_*/完整性改成
            summaryOnly），reading_state、favorite、guid、normalized_link、fallback_fingerprint、
            sync_key、feed_id 一律不出现在补丁里——「刷新不复活」是这条语句的形状保证的；
            purgeArticle 在**单事务**内删会话 → 译文（段落级联）→ 引用**置空而不删除** → 删文章
            本体，图片缓存按地址哈希在事务**外**回收（文件删除没有事务语义）；分类统计用聚合 SQL
            且**只数非空正文**；新闻引用计数用带 `%` 前导与数字边界的两组 LIKE 模式（见 R048 式的
            实现说明，实测：缺前导 `%` 永远零命中、缺数字边界会把 12 算成 1）。
      - 新增 infrastructure/local/media_cache_port_adapter.dart、degraded_storage_cleanup_store.dart
            （降级下读空、写明确失败：报成功会让用户以为敏感内容已经删除）。
      - infrastructure/local/image_cache_service.dart：新增 diskUsage（含元数据与 .part 的磁盘占用）、
            expiredUsage/deleteExpired（按 mtime 的最后访问时间，含半成品；删除时同步清内存缓存，
            否则表现为「刚清完又能读到」）。describeBytes 提升到 core（设置页要显示它，而 features
            不得 import infrastructure）。
      - infrastructure/local/ai_task_store.dart + tables/ai_tables.dart + database.dart（**schema v17**）：
            DriftAiResultCache 加 byte_length（写入时算好，统计与淘汰因此不必读全部结果文本）与
            last_used_at（命中即刷新，LRU）；**每次成功写入后自动淘汰**；新增 AiResultCacheMaintenance
            端口（与任务路径的接口分开：任务执行拿不到「按上限删一批」的能力）。迁移里 byte_length
            用 `length(CAST(result_text AS BLOB))` **回填**（length(text) 是字符数，中日韩会低估约
            三分之一），last_used_at **不回填**（历史行没有「曾经被读过」这个事实）。
      - 界面：新增 features/settings/presentation/storage_page.dart（占用分类含测量时间与媒体上限
            参照、清缓存预览→确认→回执、三个开关与天数步进（只落 1/7/30/90/365/3650 档位，越界在
            结构上不可达）、两个保护开关与警告、自动清理预览与立即执行、孤儿快照检查与回收），并在
            阅读页加「彻底删除本文」入口（先列关联范围再执行）；设置页新增「存储与清理」入口。
      - 组合根：三个新端口接线 + bootstrapOverrides 参数化（Riverpod 禁止同一 Provider 重复覆盖），
            媒体端口指向**同一个**媒体目录（否则「清缓存」会清一个空目录而占用一动不动）。
      - l10n：新增 58 条存储清理文案；l10n_test 条目数 904 → 962。
    用例与证据：新增 test/core/storage_cleanup_test.dart（14 条）、test/core/ai_cache_bounds_test.dart
      （10 条）、test/features/settings/cleanup_test.dart（19 条：**真实内存库 + 真实临时目录**）、
      test/features/settings/storage_page_test.dart（4 条组件层）、
      test/infrastructure/local/migration_v16_to_v17_test.dart（2 条，含「中文按 UTF-8 回填」与
      「时间不回填」）与 schema_snapshot_test 的 v17 断言（只加两列、不加实体、仍无凭据列）。
      **最要紧的三条断言**：清缓存后逐项检查阅读状态/收藏/订阅/付费总结/成功任务**一行未动**且两张
      AI 表**没有任何凭据列**；正文释放后用真实 `upsertArticles` 再抓一次同一 GUID，断言「没有插
      新行、状态仍是 read、正文照常写回」（**这正是「防止刷新把文章复活成未读」的可执行形式**）；
      自动清理的天数边界（91 天释放、89 天不动）与保护规则（默认跳过 2 篇、打开开关后同一批被释放）。
      全库 2119 tests 绿；dart format / dart analyze 干净。
    遗留问题与未运行项：
      1) **缺失 REVIEW**：自动清理是**不可逆**动作，其中的天数默认值（30/90/365）与保护规则的交互
         未经产品复核；另一个可选口径是「首次启用时强制跑一次预览」；
      2) **AI 结果缓存上限没有对应 SET 编号**：它是内部资源策略（写在 AiCacheLimits 的默认值里），
         不是用户可配项。若产品要求用户可调，需要扩 SET 表并重新做同步投影；
      3) **孤儿快照 GC 未在真实 WebDAV 服务器上验证**（无凭据）：判定与「预览不删」在内存服务器
         与替身上验证过，真实服务器的 getlastmodified 行为（缺失/时区/分钟精度）未实测；
      4) **正文释放不改 `body_completeness` 的历史语义**：统一改成 summaryOnly 是「与当前数据
         一致」的取值，但它会让 T039 的「原文已清理」提示只在释放前是 sourceBody/extracted 的
         行上出现，其余（本来就是 summaryOnly）不显示——这是刻意的，改判定条件会误报；
      5) **未做真实的磁盘不足/只读目录演练**：失败分类（StorageError 的 operation 名）由用例
         构造替身覆盖，真实权限错误的表现未测；
      6) 真实 WebDAV 双客户端验证仍为 NOT_RUN（无凭据，延续 R044）；其余同 R046（macOS 无后台
         执行保证、Android 未初始化、未 push）。
    需求是否变化、维护者是否批准：未改变任何验收条件文字；只更新任务状态列（T047 → DONE）、功能
      账本、R047 记录与 l10n 计数。未删除任何功能或数据。
    提交/差异范围：提交 "T047: storage cleanup with category bounds, protection rules and orphan GC"；
      基线为 T046 的提交（618c714）。未 push
    下一可执行任务及前置条件：T048（故障恢复编排与诊断导出），前置 T029/T043/T046/T047 均 DONE

### 7.1.42 轮次记录 R048（T048）

    轮次/日期：R048 / 2026-09-22
    任务 ID 与状态变化：T048（故障恢复/诊断包与隐私审计）TODO → DONE（缺失 REVIEW：本轮把
      「数据目录切换」这一跨重启的破坏性动作自动化了，并定义了诊断包的内容边界）；
      **M3 出口条件达成（T041–T048 全部 DONE，M3 完全闭环）**
    相关决策/功能/SET 项：SET-082（诊断日志级别/保留天数，按需脱敏导出、不含原文与 prompt）、
      架构 5.3 的恢复段（「恢复默认到新数据目录，先校验 schema、大小、路径、哈希，失败不动原库；
      正式切换前再次确认」）、架构第 8 节（「日志/截图/分享/备份不得泄露凭据」「诊断预览脱敏」）
    修改文件与主要行为：
      - 新增 core/domain/restore_orchestration.dart（**纯规则**）：RestorePhase（pendingRestart/
            switched/cleaned 三阶段即那次恢复意图的完整生命周期）与 RestoreMarker（**文件名写在
            数据目录的父目录**——写在数据目录内部会让「数据目录被改名」这一步顺带把标记搬走，而它
            恰恰要在搬移过程中一直可见）；decode 对任何结构问题（坏 JSON、不认识的阶段、空路径、
            坏时间）返回 null 而不是凑一份默认值（凑出来的标记会让应用把数据目录切到不存在的位置）；
            decideRestoreAction 是四事实的纯函数：**switched 阶段绝不再切一次**（那会把正在用的
            目录搬到别处）、pendingRestart 但恢复目录不存在或库不可用时丢弃标记并报告、恢复目录
            就是当前目录时按规范化路径判等后不搬；RestoreFailureKind 六个稳定类别名；
            RestoreOrchestrationState（含 needsCleanupConfirmation「有停放目录才算待清理」）。
      - 新增 core/domain/diagnostics_export.dart（**纯规则**）：DiagnosticsAllowlist 四节**字段白名单**
            （system/storage/sync/log），与 DiagnosticsReportBuilder（白名单之外的字段**直接丢掉**
            而不是报错或写进去；值逐条脱敏并单行化；**最后整篇再跑一遍**脱敏——拼装会引入「单看每段
            都干净、连起来形成 `名字=值`」的形状）。
      - 新增 features/settings/application/maintenance_ports.dart：RestoreOrchestrationPort（读/写/
            清标记、目录存在性、库可用性、**搬移（目标已存在必须失败）**、删除目录、停放目录命名）、
            DiagnosticsExportSource（四节）；两者的签名里都没有「就地覆盖」与「任意文本」的入口。
      - 新增 features/settings/application/restore_orchestrator.dart：markPendingRestart（由 T046 的
            恢复成功路径调用）、readState、**runOnStartup**（顺序：先判目录可用性再决定；切换 =
            停放旧目录 → 启用新目录，第 2 步失败**搬回来**并保留 pendingRestart 标记供下次重试，
            回退本身失败则明确报错而不是静默停在停放位置；只有「恢复目录不可用」这一种失败会清标记，
            否则每次启动都会重复报告一个无法完成的任务）、completeCleanup（只在 switched 阶段生效，
            删不掉时**保留标记**）、abandon（只清标记，**不删**恢复出来的那份数据）。
      - 新增 features/settings/application/diagnostics_export_service.dart：buildReport（四节 → 白名单
            构建器）与 export（复用 T015/T046 的 FileAccessPort，不新增任意写盘通道；文件名只带日期
            ——设备名会出现在邮件标题里，是第二条说明「这是谁的机器」的线索）。
      - 新增 infrastructure/local/restore_orchestration_store.dart：标记先写 `.part` 再 rename（半截
            标记会被读成损坏并丢弃，恢复意图静默消失）；databaseUsable **真的打开一次库**再关掉
            （「有一个 flux.sqlite」与「这是一个能用的库」是两件事，而切换之后才发现它坏了）；
            moveDirectory 先显式判断目标是否存在（`Directory.rename` 在目标为空时成功、非空时失败，
            两种都不可依赖；静默合并不可撤销）。
      - 新增 infrastructure/local/diagnostics_export_source.dart：四节逐条来自**既有读取路径**
            （存储统计复用 T047 的 measureDatabase、同步摘要复用 T044 的窄读取端口、日志复用
            DiagnosticLog.export 的「导出时再脱敏一次」）；**没有任何一节读过 articles.body /
            ai_tasks.input_snapshot / news_runs.prompt**——不是「本文件没写」，而是「本文件拿不到」
            （注入的端口里没有正文读取口）；系统信息只报**数据目录是否存在**（不报路径，路径里有
            用户名）；语言与主题走 settingsSummary 回调读**有效值**（读不到报 unavailable，不填占位
            字符串）；同步摘要**不报服务器地址与用户名**。
      - lib/app/app_bootstrap.dart：恢复编排放在**打开数据库之前**（先开旧库再改名会让数据库文件
            在使用中被搬动），编排阶段的内存日志条目在建立文件 sink 时**搬过去**（否则「恢复编排
            做了什么」在诊断包里凭空消失）；AppBootstrapResult 增加 restoreAction/restoreFailureKind。
      - 界面：新增 features/settings/presentation/diagnostics_page.dart（导出小节 + 恢复编排状态：
            三种阶段各自说明「当前数据在哪」，**只在有停放目录时**给出「删除旧数据目录」，pendingRestart
            阶段只给「放弃这次恢复」；导出前确认页明说包内有什么、不含什么），设置页新增「诊断与恢复」入口。
      - 组合根：两个新端口接线 + bootstrapOverrides 参数化；restoreActionProvider 承载启动结论
            （界面读它而不是重读标记——重读会带上「清掉残留 cleaned 标记」这类副作用）。
      - l10n：新增 27 条诊断与恢复文案；l10n_test 条目数 962 → 989。
    用例与证据：新增 test/core/restore_orchestration_test.dart（11 条）、
      test/core/diagnostics_export_test.dart（10 条：白名单丢弃、四类秘密脱敏、单行化、整篇重扫）、
      test/features/settings/restore_orchestration_test.dart（11 条：**真实临时目录 + 真实库**，覆盖
      「切换后当前目录装的是恢复数据、旧目录停在并列位置且数据完好」、switched 再启动不再切、
      恢复目录不存在/库损坏两种失败都保持当前数据不动、切换失败三步（停放→启用失败→回退）、
      回退失败明确报错且数据在停放目录里可找回、放弃只清标记不删数据、目标已存在拒绝合并）、
      test/features/settings/diagnostics_export_test.dart（8 条：**真实四节内容**装进包后断言无原文、
      无 AI 产出、无 prompt、无密钥、无订阅地址；脱敏幂等；数据库不可用时仍能导出且如实报 unavailable）、
      test/features/settings/diagnostics_page_test.dart（4 条组件层）与设置页入口贯通 1 条。
      「脱敏二次检查」的落点是**幂等断言**（对已脱敏文本再脱敏必须不变）——若它还变，说明第一次
      留下了可继续识别的秘密碎片。全库 2164 tests 绿；dart format / dart analyze 干净；
      `flutter build macos --debug` 退出 0。
    遗留问题与未运行项：
      1) **缺失 REVIEW**：数据目录切换是跨重启的破坏性动作（本轮把 T046 的「需重启」半自动编排
         起来了），而「重启前是否要让用户再看一次确认」「旧目录是否默认保留」这两个口径未经产品
         复核。当前实现：切换由启动路径自动完成（不再问一次），旧目录**默认保留**直到用户确认删除；
      2) **无法自动重启进程**：macOS 上应用不能自行重启，因此 pendingRestart 状态需要用户手动重启
         才生效；界面明确说明了这一点，但「编排完成的即时反馈」只能到「已就绪，等重启」为止；
      3) **诊断包未做「预览后导出」的两步式**：本轮是「说明 → 确认 → 保存」，没有把生成好的文本先
         展示给用户看一遍再导出（那会引入一份明文副本的显示与内存驻留）。若产品要求，应作为独立任务；
      4) **日志层的边界**：诊断包的结构性内容（四节）在**类型上**拿不到原文与 prompt；但日志小节的
         文本来自调用方，其契约是「凭据被脱敏」（T010），**不是**「认得出任意一段散文是不是服务器
         正文」。因此写入日志的正文类内容不会被识别——实际影响是通过「代码审阅 + 逐字段白名单」
         控制的，而不是靠脱敏正则。这一点在 diagnostics_export_test.dart 与 R048 里如实写明；
      5) **Android 的数据目录切换未验证**（沙盒路径与应用私有目录在移动端的行为不同，且没有
         等价于 macOS「并列目录 + 重启」的体验）；属 T050；
      6) **未做真实的磁盘不足/权限拒绝演练**（同 R047 第 5 条）；真实 WebDAV 双客户端验证仍为
         NOT_RUN（无凭据，延续 R044/R047）。
    需求是否变化、维护者是否批准：未改变任何验收条件文字；只更新任务状态列（T047/T048 → DONE）、
      功能账本、M3 出口声明与阶段摘要、R047/R048 记录与 l10n 计数。未删除任何功能或数据。
    提交/差异范围：提交 "T048: restore orchestration with rollback and sanitized diagnostics export"；
      基线为 T047 的提交（333707d）。未 push
    下一可执行任务及前置条件：**M3 已闭环**；下一阶段为 M4（T049 macOS 专项验收、T050 Android
...
### 7.1.44 轮次记录 R050（T050）
    ...
    轮次/日期：R050 / 2026-09-22
    任务 ID 与状态变化：T050 在手册里是「Android 专项验收」。本轮任务单把 T050 指定为「资源占用
      基线与性能验证（macOS 范围）」，即**手册 T052 的固定数据集与性能部分**。因此：
        a) **手册的 T050（Android）保持 TODO**：android/ 工程仍未初始化、无设备、无 SDK 核对，
           逐项 NOT_RUN，不冒充通过（任务表已在 T050 行显式标注这一分歧）；
        b) **本记录报告的是实际执行的内容**：架构第 8 节的数据集与基线测量（macOS，debug）。
      为避免把两件事混成一条记录，性能基线的实测值写入 §7.2 的「包体/内存/启动/能耗基线与阈值」行，
      本记录只写口径、方法、结论与遗留。
    相关决策/功能/SET 项：架构第 8 节「性能验收集：100 个订阅、50000 条文章、100000 字符长文；
      首屏/已索引本地搜索 P95 初始目标不超过 1 秒；60Hz 常规滚动多数帧 16.7ms 内；包体积、安装
      大小、冷启动、静置/阅读内存、峰值、能耗必须记录」、手册 6.4（未批准阈值不可写通过）
    修改文件与主要行为：
      - 新增 lib/infrastructure/local/perf_dataset.dart：验收集生成器（可调规模，默认
        100/50000/100000）。放在 infrastructure 而不是 tool/ 的理由：数据集必须经过与生产**完全
        相同**的表结构、索引与 FTS 触发器，否则测出来的数字与实际运行的不是一回事。用**固定种子**
        的线性同余发生器（不用 Random(seed)：它的算法是库的私有约定，跨 Dart 版本可能变化，
        而基线要能与下一次比较）。分批 500 行一个事务插入——50000 条逐条插入要几分钟，
        而**批量插入同样执行 FTS 触发器**（触发器是表级行为，与插入方式无关），索引因此是真实建
        起来的。
      - 新增 test/perf/perf_baseline_test.dart：三条**默认回归**用例（数量与长文长度、同一 spec 的
        确定性与不同 seed 的差异、FTS 索引真实建起来且真检索一次有命中）+ 一条**默认跳过**的采集
        用例（--dart-define=PERF_SEED_DATASET=true 才跑；默认跑会让每次 flutter test 都慢几十秒）。
      - 新增 test/perf/perf_frames_test.dart：首屏到首帧、稳态滚动帧、加载下一批那一帧，
        以及**帧耗时是否随已加载条数增长**。
      - 两条 self-check 是本轮最有价值的产出，都是先测出**假数字**才补上的：
        a) 最初用 find.byType(Scrollable).first 取滚动位置，取到的是工具栏里某个不可滚动的
           Scrollable（maxScrollExtent = 0），jumpTo 成了空操作，测出来的「帧耗时」是 0.019ms
           的纯空转。补上 maxScrollExtent > 1000 的断言后立刻失败并暴露了这个问题。
        b) 断言「已加载 100 条但构建的卡片数 < 60」——虚拟化失效是二值事实，比「帧耗时变差多少」
           这个连续量更难被忽略（有人把 ListView.builder 换成 Column + SingleChildScrollView 时）。
      - 「随已加载条数增长」这一段改过一版：第一版在列表底部测量，而接近底部会**触发下一批加载**，
        把一次几千行的数据库读取算进了帧耗时，得到 1.7 秒的假数字、用例跑了 83 秒。改成「只在顶部
        附近测、测量与加载严格分开」后数字稳定在 8–10ms 且**逐轮下降**（JIT 预热）。
      - 另需说明 bool.fromEnvironment 的一个坑：它只认字面量 true/false，传 1 会**静默**得到
        false。第一次触发采集时用例报「未设置 PERF_SEED_DATASET=1，跳过」正是这个原因。
    实测（macOS 27.0 / M4 / 16 GiB / arm64；Flutter 3.47.0 / Dart 3.13.0；**debug**）：
      - 数据集：100 订阅 / 50001 文章 / 长文 100000 字符，生成耗时 5.1 s；
      - 冷启动到首帧：**498 ms**；
      - 列表首批 100 条读取：P50 13.5 ms / P95 **16.0 ms**；
      - FTS 检索 P95：常见词（「离线阅读」，命中很多）**54.4 ms**、稀有词 26.3 ms、单字（LIKE 路径）
        21.2 ms——三项都远低于「P95 < 1 s」的初始目标；
      - 稳态滚动帧：P50 **23.2 ms**、P95 66.8 ms，40 帧中 28 帧超 16.7 ms（**未达**「多数帧 16.7ms 内」）；
      - 滚动到底触发下一批那一帧：P50 **159.6 ms**（6/6 帧超预算）；
      - 帧耗时与已加载总量的关系：6 轮顶部中位数 9.71 → 8.02 ms（**不随**加载增长，虚拟化有效）；
      - RSS 峰值：**391 MiB**；FTS 索引占用（dbstat）：**71.2 MB**。
      - 能耗：**NOT_RUN**（macOS 无公开 API，与手册口径一致，不猜数值）。
    优化：**未做针对性修改**。本轮没有发现「应该优化」的确证瓶颈，理由逐条如下，不做没有依据的改：
      a) 滚动帧超预算（28/40）：这与 50000 行无关（下一批加载才慢，稳态帧耗时逐轮下降），
         因此**不是列表规模问题**，而是 debug 模式下单个卡片的构建/布局成本（assert、
         无 AOT、flutter_svg 等）；判定它是否真实存在**必须**有 release 数据，而 release 属 T052/T054。
         在 debug 数字上做「优化」无法证明有效，还可能掩盖真实瓶颈。
      b) 加载下一批那一帧 159.6 ms：一次读 100 行 + 构建 100 张卡片，同样是 debug 成本，
         且发生在用户滚动到底的时刻（可感知但非持续）。若 release 下仍超预算，正确的修法是
         分批构建或减少首帧构建量，属 T052 的阈值审批之后。
      c) 检索与首批读取都已远低于目标，没有优化空间可谈。
    用例与证据：test/perf/ 下 5 条用例（3 条默认回归 + 2 条默认跳过），默认全库 **2181 通过 /
      4 跳过 / 0 失败**（相对 T049 的 2176 新增 5 条）；采集结果落在 build/perf_baseline.json 与
      build/perf_frames.json（git 忽略的构建目录），实测值已写入 §7.2。
    遗留问题与未运行项：
      1) **阈值未批准**：手册 6.4「已批准阈值通过，未批准阈值不可写通过」。本轮只记录实测值，
         **不宣称达标**；尤其「滚动多数帧 16.7ms 内」在 debug 下**未达成**。绝对阈值审批与
         release 复测属 T052；
      2) **能耗 NOT_RUN**：macOS 无公开能耗 API（与手册既有口径一致），不用推断值顶替；
      3) **全部数字是 debug 模式**：绝对耗时与内存都偏悲观，**不能**作为对外性能声明。手册 §7.2
         已逐项标注；profile/release 对比属 T054；
      4) **数据集是合成文本**：生成器产出的是结构化假数据（与真实中文语料的词频分布不同，
         检索耗时会随命中数变化）。它覆盖了「行数/索引规模/长正文」这些形状，但不是真实抓取语料；
      5) **包体积与安装大小未测**：本轮测的是运行时指标；包体积属 T054 的发布构建（且需 release）；
      6) **内存只报了峰值（RSS）**：静置内存与阅读内存的**分项**未单独采样（只有一个进程峰值）。
         release 下的分项采样属 T052；
      7) **Android 侧的性能完全未测**（工程未初始化）；FTS 的 trigram tokenizer 在 Android 内置
         SQLite 上是否存在仍未验证（R022 已记同一条）；
      8) **未做真实设备的长文排版帧测量**（框架内测的是列表与首屏；十万字长文的排版耗时未单独
         采集，只有它躺在库里作为数据量的一部分）；
      9) 本轮成果未 push 到远端。
    需求是否变化、维护者是否批准：未改变任何验收条件文字。**但记录了一处任务号与范围的错位**：
    ...
    下一可执行任务及前置条件：T051（全设置审计与中英/浅深/可访问性）与 T052（性能/资源/安全
      回归，需先由维护者批准阈值）；T049 遗留的 profile/release 与人工专项、T050 的 Android 部分
      仍待硬件与授权
### 7.1.43 轮次记录 R049（T049）

    轮次/日期：R049 / 2026-09-22
    任务 ID 与状态变化：T049（macOS 专项验收）TODO → DONE（**范围收窄 + 缺失 REVIEW**）。手册 T049
      行的原文要求覆盖「菜单/输入法/键盘/窗口/权限/Keychain/后台生命周期，profile/release 记录」；
      本轮在授权范围内交付**键盘与可访问性**这一组，其余各项**未做且不冒充通过**（逐条见下）。
    相关决策/功能/SET 项：架构第 7 节（键盘焦点/右键/菜单/快捷键/触控目标/动效/读屏标签/大字号/
      窄窗）、SET-006（字号上限）、SET-014（减少动态效果，本轮从「仅注册表」变为真实生效）、D-02
      （macOS 优先，快捷键用 ⌘ 的依据）
    修改文件与主要行为：
      - 新增 app/shell/app_shortcuts.dart（快捷键的**意图与按键分离**）：四个 Intent
        （SwitchDestinationIntent / ShowShortcutHelpIntent + 复用的 DismissIntent）、⌘1/2/3 与 ⌘/
        的按键映射、说明面板的内容清单（独立的条目表，测试能逐条断言）与 AlertDialog；
        **关键实测结论**：应用级 Shortcuts 必须挂在 **MaterialApp 之外**。按键事件从当前焦点结点
        沿**祖先链**向上找处理者，而没有控件聚焦时 primaryFocus 是路由的 FocusScopeNode；
        挂在 AppShell（路由的**后代**）上时事件永远走不到它——现象是「⌘2 毫无反应」，而 Shortcuts
        看起来挂得好好的。第一次实现就踩了这个坑，用临时探针用例打印 primaryFocus 才定位。
        说明面板本身需要 Navigator（对话框），因此那一层只置一个**请求**（shortcutHelpRequest），
        由壳层（Navigator 内部）监听并弹出——与 T020 的「去设置」同一条路线。
      - 列表键盘与可访问性（article_list_view.dart）：新增 _KeyboardLayer。同样受上面那条约束：
        它**必须自己拿到焦点**（canRequestFocus: true + autofocus）。最初写成 canRequestFocus: false
        是错的——那样没有焦点结点，事件只会从路由的 FocusScope 向上走，同样到不了它。
        上下键移动落点（夹取不环绕）、回车打开落点那篇、Esc 逐层退出（批量模式 → 键盘落点，
        **一次只退一层**且没有可退时返回 ignored 让事件继续冒泡）；落点行用 FluxCard 新增的
        keyboardFocus 画强调边框（与批量勾选的 selected 视觉分开：一个是「会参与批量操作」，
        一个是「回车会打开它」）；移动时用 SemanticsService.sendAnnouncement 播报「第 N 篇 / 共 M 篇」
        （用 sendAnnouncement 而不是已弃用的 announce：后者不区分窗口）。
      - 右键菜单三处：列表项（打开/浏览器打开/三态直达/收藏/**彻底删除**）、正文区（系统复制
        菜单 + 解释 + **在库检索** + **翻译本文**）、订阅行（复用行尾 PopupMenuButton 的 showButtonMenu，
        菜单内容因此只有一处定义——另建一份动作表必然慢慢漂移）。列表项的「彻底删除」复用 T047 的
        CleanupService：先列关联范围、再确认、再执行，与详情页入口同一套用例，不复制删除逻辑。
      - 正文「在库检索」（新增 application/reading_search_request.dart）：与「解释」分开是**故意的**
        ——解释把选区发给 AI 服务（出网、可能计费），检索只查本机数据库。两者代价不同，
        因此是两条明确的菜单项。实现走「置请求 → 壳层切去向 → 阅读页消费并检索一次 → 清空请求」。
      - Esc 退出搜索（reading_page.dart）：Focus 包在**搜索框外面**而不是整页。理由同前，
        按键只沿焦点祖先链投递，而搜索框是这里唯一会聚焦的控件；挂在页面上永远收不到。
      - 读屏语义：图片占位合成一个语义节点（图标+替代文字+失败说明分开播报会变成碎片）、
        列表加载中/详情加载中加 liveRegion 标签。三态与收藏的标签 T012 已有，本轮加断言钉住。
      - SET-014 真实生效（新增 settings/application/motion_preference.dart + presentation/motion_scope.dart）：
        system 档解析为 **null（不覆盖）**而不是某个 bool 兜底——用一个 bool 会把「没有意见」写死
        成 false，等于忽视系统设置（macOS 上用户开了 reduce motion 而我们的转场仍在播）。
        覆盖点在 MaterialApp.builder（路由及其后代都在这个 MediaQuery 之下），因此框架的页面转场、
        涟漪与 Material 控件的动画一起服从；system 档则**原样返回子树**，不 wrap 一层快照，
        否则会切断子树对系统设置后续变化的响应。设置页的可见控件属 T051（本轮只做真实生效）。
      - 大字号与窄窗：test/app/test_harness.dart 的 wrapFluxApp 新增 textScaler 参数（默认不缩放）。
    用例与证据：新增 test/app/platform_experience_test.dart（12 条：SET-014 合成两条纯规则、
      快捷键映射与说明条目两条纯规则、⌘1/2/3 切换三去向、⌘/ 弹面板且 Esc 关闭、上下键落点 + 回车
      打开、三态/收藏/卡片的语义标签、窄窗 599 不溢出、大字号 2.0 倍控件不被截断、右键菜单六项内容、
      Esc 退出批量模式）与 test/features/articles/golden/a11y_regression_golden_test.dart（2 张 golden：
      大字号列表、窄窗 599 单栏；**人工确认过**两张图里每张卡片右侧的三态与收藏都完整可见）。
      l10n 新增 31 条中英（快捷键面板与分组、读屏标签、选区菜单、列表项菜单、图片占位、SET-014 文案），
      条目数 989 → 1020（l10n_test 的计数断言同步更新）。全库 **2176 tests 绿**；dart format /
      flutter analyze 干净；flutter build macos --debug 退出 0。
    遗留问题与未运行项：
      1) **缺硬件/需人工的专项未做**（本任务原文要求的范围大于本轮交付）：
         a) **profile/release 记录未做**：本轮全部证据是 debug。性能绝对值受 debug 影响很大
            （见 R050），release 对比留 T054/T052；
         b) **macOS 原生菜单栏未做**（应用菜单、编辑菜单、窗口菜单），本轮做的是应用内快捷键；
         c) **输入法（中文 IME）专项未做**：未在真实 IME 下验证组合输入与快捷键是否冲突；
         d) **窗口与后台生命周期专项未做**（最小窗口尺寸、全屏、多窗口、挂起恢复）；
         e) **权限与 Keychain 专项未在本轮复测**：T010 的真机 Keychain 往返仍有效，但未针对本任务重跑；
         f) **真实 VoiceOver 人工走查未做**：本轮是语义树断言（结构性证据），不等于读屏实际体验。
         上述六条按手册 6.4 记为 **REVIEW**，不写成通过。
      2) **Tab 焦点顺序未做系统化审计**：本轮依赖控件自身的焦点基础（T012 已有）与两次实测出的
         「焦点必须在按键处理者的祖先链上」这条结构约束，但没有逐页走一遍 Tab 序列并断言顺序。
         该审计属 T051（全设置审计与可访问性）。
      3) **说明面板只列了本轮实现的快捷键**：macOS 应用菜单（⌘W/⌘Q 等系统项）未列入，
         因为那些不在应用控制范围内。
      4) Android 工程仍未初始化；本轮成果未 push 到远端。
    需求是否变化、维护者是否批准：**验收条件文字未改**，但 T049 的**实际执行范围小于其原文**
      （原文含菜单/输入法/窗口/权限/后台生命周期与 profile/release 记录）。这是本轮的授权范围
      （任务单明确写「平台体验与可访问性，macOS 范围」）与手册行文的差异，此处显式记录并保留
      REVIEW，未把未做项写成通过。若需按原文完整收口，应作为独立任务补充（需真实硬件与人工操作）。
    提交/差异范围：提交 "T049: keyboard navigation, context menus, semantics and a11y regressions
      (macOS)"（110d9ca）；基线为 T048 的提交（4275b2e）。未 push
    下一可执行任务及前置条件：T050 的 Android 部分（需 android/ 工程初始化与设备）与 T051；
      T052 的性能部分已在本轮 T050 单中提前执行，见 R050
...
### 7.2.3 T047/T048 新增的运行时机制（均无第三方依赖）

| 能力 | 实现位置 | 实测/依据 |
| --- | --- | --- |
| 容量双上限与 LRU 淘汰（T047） | core/domain/ai_cache_bounds.dart、infrastructure/local/ai_task_store.dart、schema v17 | **无第三方依赖**：判定是纯 Dart（条数 + 字节两个条件都要满足、按最近使用升序、同一时刻按键排序保证确定性），UTF-8 字节数自实现（不引入 convert 之外的依赖；CJK 3 字节、代理对 4 字节有用例逐字符类比对）。写入时算好 byte_length，统计与淘汰因此不必把全部结果文本读进内存（上限 128 MiB） |
| 迁移里的字节回填（T047） | lib/infrastructure/local/database.dart（v16→v17） | 用 SQLite 的 `length(CAST(result_text AS BLOB))` 一次性回填。**实测差异**：直接 `length(result_text)` 是**字符数**（「你好世界」= 4），CAST 成 BLOB 后才是**字节数**（= 12）；迁移用例对同一行逐值断言，与 Dart 侧 utf8ByteLength 口径一致 |
| 正文释放保身份（T047） | infrastructure/local/storage_cleanup_store.dart（releaseArticleBodies） | **无第三方依赖**：一条 UPDATE 只写正文列与完整性，身份/状态/同步键列不出现在补丁里；「刷新不复活」由真实 `upsertArticles` 再抓同一 GUID 的用例证明（inserted=0、状态仍 read、正文写回） |
| 远端快照的孤儿 GC（T047，T042 遗留） | core/domain/storage_cleanup.dart（orphanSnapshotsToCollect） | 判定是纯函数（未被引用 + 超过保留 90 天 + 拿得到服务器时间三者同时成立）；「预览不删」由替身记录删除调用断言。**真实服务器的 getlastmodified 行为未实测**（无凭据） |
| JSON 素材里的引用计数（T047） | infrastructure/local/storage_cleanup_store.dart（_countNewsReferences） | **实测两条 SQLite 行为**：① LIKE 是整串匹配，模式缺**前导 `%`** 时永远零命中（`LIKE '"articleId":1,%'` 返回 0，`LIKE '%"articleId":1,%'` 返回 1），而这个失败没有任何报错——表现为「确认页说没被引用、删完却发现引用断了」；② 数字后缺**边界**时会把 articleId=12/13/100 全算成 1（同一条数据返回 2）。因此用「前导 `%` + 逗号或右花括号两种边界 + 带/不带空格两种编码」四组模式。结果只用于确认页提示，漏报不造成数据损失 |
| 恢复编排的目录切换（T048） | features/settings/application/restore_orchestrator.dart、infrastructure/local/restore_orchestration_store.dart | **无第三方依赖**：只用 dart:io 的 `Directory.rename/delete` 与 `path`。**刻意不用「删除 + 改名」**：改名失败时旧目录还在（可回退），删除不可撤销。**实测 `Directory.rename` 的两种行为**：目标为空时成功、非空时失败——两种都不可依赖，因此实现先显式判断目标是否存在再改（静默合并不可撤销）。库可用性用真的 `openAppDatabase` 打开一次判定并随即关闭（长期持有会让后续改名失败） |
| 诊断包的字段白名单（T048） | core/domain/diagnostics_export.dart | **无第三方依赖**：白名单是四张静态字符串表，构建器对白名单外的字段**直接丢掉**；脱敏复用 core/error/secret_redaction.dart（T007）。用例断言导出文本**幂等**（对已脱敏文本再脱敏不变），并逐条断言真实四节内容里不含原文/AI 产出/prompt/密钥/订阅地址 |
| 输出文件的原子写（T047/T048） | infrastructure/local/restore_orchestration_store.dart | 恢复标记先写 `.part` 再 rename：半截标记会被解析成损坏并丢弃，而「恢复意图静默消失」比多一个临时文件严重得多。与 T021 的图片缓存、T046 的备份快照同一手法 |
...
| WebDAV（同步传输，T042） | **不是** AI/搜索提供商，而是**用户自配的同步服务**（Nextcloud/坚果云/自建 Apache mod_dav 等）；协议动作 = PROPFIND/GET/PUT/DELETE/MKCOL + Basic 认证；能力探测四步见 R042 | `test/infrastructure/network/fake_webdav_server.dart` 是**状态化**内存服务器（文件表 + ETag + If-Match + 412 + MKCOL/PROPFIND/GET/PUT/DELETE + 断网开关），45 条用例覆盖探测六种判定、XML 防护、请求往返、发布协议、冲突重试、孤儿快照与 manifest 编解码；T043/T044 另加 5 + 10 条（能力探测四类服务器判定与只读探测） | **NOT_RUN**（无任何 WebDAV 凭据；也未向任何真实服务发起请求；T043/T044 同样只对内存服务器发起请求）。因此「某个具体服务在两台设备并发时是否真的按强 ETag/If-Match 行为」**没有实测证据**；测试证明的是协议规则与客户端行为，不是服务器兼容性 | **否**（协议实现、合并规则、发布编排与设置页均完成，但服务器兼容性未验证） |
...
### 9.1 R-BUILD 预发布构建与清理记录（用户指令）

日期：2026-09-22。执行者：主代理（Codex）。

用户指令：关闭残留子代理与构建进程；删除 m0 工作目录；清理缓存与构建；更新任务状态；构建 macOS 与 Android 客户端供检查；推送 GitHub。

执行事实：

1. 子代理：Ptolemy（A17）确认早已关闭（not_found），其余代理均已关闭；gradle/java/flutter_tester/dart 后台进程全部终止，残留 0。
2. 清理：Flux-m0-workspace（8MB，evidence 截图与 logs）移入废纸篓（可恢复）；flutter clean 后主工程从 3.7GB 降至 48MB；早期原型工程 Flux-prototype-verify（657MB）已于 R-PAUSE 移入废纸篓。
3. Android 初始化（新）：flutter create --platforms android；applicationId/namespace 改为 io.github.guzhengsvt.flux（与旧版 com.example 占位不同，避免升级覆盖）；MainActivity 迁移至匹配 Kotlin 包路径；android:label=Flux；删除遗留的模板 widget_test.dart（引用不存在的 MyApp，T011 起即为过期文件）。
4. 构建产物（交付检查，位于 /Volumes/taurus/Document/Code/Flux-dist-check/）：
   - Flux-android-arm64.apk：27.0MB（26989536 字节），release，target android-arm64，debug 外发签名（不可上架，可安装测试）
   - Flux-macos.zip：13.2MB 压缩（Flux.app release 展开 31MB）
   - SHA-256 全值见构建时输出；macOS 与 Android 均为 release 模式
5. 测试：Android 初始化后全量 flutter test 2199 通过 / 4 跳过 / 0 失败（删除 widget_test 后 2197+2 计数口径以 CI 输出为准）；flutter analyze 无问题。
