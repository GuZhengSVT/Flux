...
| 新版软件功能实现 | DOING | M0 骨架已就绪；M1 已 DONE 的九项：T009（SQLite/Drift 实体、索引、事务及迁移）、T010（设置注册表 SET-001–084、schema v2 真实增量迁移、macOS Keychain 安全存储、脱敏诊断）、T011（应用壳、三去向导航、首次引导、主题 token 与中英 i18n）、T012（原创 SVG 图标集、三态阅读控件与独立收藏、空态/状态横幅/统一卡片与八类状态）、T013（RSS/Atom 抓取、安全解析、身份去重与正文清洗，schema v3）、T014（单源添加/编辑/启用/加精、分组增删改/排序/置顶/折叠、未分类保护，订阅管理页）、T015（OPML 导入预览/逐项结果/重试与导出，URL 秘密参数排除）、T016（刷新调度：启动/定时/手动触发合并、并发 4、失败隔离、离线与计费网络守卫）与 T017（三态阅读/收藏、四类筛选与分页列表、批量范围操作与撤销、打开正文自动标已读）。现状：可启动真实应用并浏览真实文章列表——RSS 去向现在是完整可用的阅读页（筛选、分页、三态、收藏、批量、刷新），「今日新闻」仍是明确占位；「我的」页设置区可进入订阅管理页完成添加/编辑/分组与 OPML 导入导出；SET-001 语言、SET-002 主题真实读写并即时生效，其余 SET-003–016 只以禁用态展示、无假开关；浅/深两套 ThemeData 由架构第 7 节 token 表生成；T013 的抓取/解析/清洗已在真实源上跑通（见 7.1.6）；正文阅读器（标题排版、代码、公式、目录、上下篇）仍属 T019，当前详情页是最简占位（标题 + 纯文本 + 关闭）。真实 macOS 窗口（1200×832 内容区）已截图入证据 |
...
当前阶段：M1 进行中，T001–T017 已完成并有本机证据；下一任务 T018（删除订阅/组与保留收藏策略），前置 T017 已 DONE。当前阻塞：无文档阻塞；Android 工程（含 Keystore 实测）与两平台正式签名仍未执行，须在对应任务获取授权后处理，不伪造完成记录。
...
| 文章三态/收藏/批量操作 | F-STATE；SET-010、081 | T017、T018、T045 | DOING（T017 交付：三态与收藏的真实写入与列表操作、四类筛选（later 独立入口）、发布时间倒序分页（缺失用抓取时间并注明）、批量范围（全部/筛选结果/所选行）与批量标已读/未读/later/收藏、批量操作可撤销（按快照逐行恢复）、SET-010 打开正文自动标已读（later 不动）；除订阅时的状态同步与删除订阅的收藏保留仍属 T018/T045）|
